...
type
  ...
  public
    ...
    procedure InsertTable(X, Y: Integer; Link: Boolean);
    function InsertLink(_tbl1, _tbl2: TOQBTable; _fldN1, _fldN2, _type: Integer): TOQBLink;
  ...
  TOQBForm = class(TForm)
    ...
    procedure QBTablesDrawItem(Control: TWinControl; Index: Integer;
...
uses
  QBLnkFrm, QBAbout, Estrutura_Case, Abertura, Rotinas, Gera_01;

{$R *.DFM}

resourcestring
  sMainCaption = 'Diagrama';
...
begin
  ...
  begin
    ...
    TOQBForm(FOQBForm).btnOpenClick(Self);
...
begin
  ...
  begin
    ...
    begin
      ...
      if Integer(Items.Objects[itemID]) = 1 then
      begin
        Canvas.Font.Style := Canvas.Font.Style + [fsUnderline];
        Canvas.Font.Color := clBlue;
      end;
      if Integer(Items.Objects[itemID]) = 2 then
      begin
...
begin
  ...
  //    end;
  //end;
  //OQBEngine.FieldList.Clear;
  p := TOQBForm(GetParentForm(Self)).QBTables.Items.IndexOf(ATableName);
  if p > -1 then
    TabGlobal_i.DTABELAS.GotoBookmark(TOQBForm(GetParentForm(Self)).QBTables.Items.Objects[P]);
  ...
  begin
    if TabGlobal_i.DCAMPOST.CHAVE.Conteudo = 1 then
      FLbx.Items.AddObject(NomeFisicoCampo, TObject(1))
    else if TabGlobal_i.DCAMPOST.EXTRA.Conteudo = 1 then
      FLbx.Items.AddObject(NomeFisicoCampo, TObject(2))
    else
      FLbx.Items.AddObject(NomeFisicoCampo, TObject(0));
...
begin
  TOQBForm(GetParentForm(Self)).FModify := True;
    ...
    begin
      ...
      if Source <> FLbx then
        TOQBArea(Parent).InsertLink(
          ...
          TOQBTable(TWinControl(Source).Parent).FLbx.ItemIndex, nRow, 0)
      else if nRow <> FLbx.ItemIndex then
        TOQBArea(Parent).InsertLink(Self, Self, FLbx.ItemIndex, nRow, 0);
    end
    else
      ...
      begin
        ...
        TOQBArea(Parent).InsertTable(X, Y, True);
      end;
  end
end;
...
begin
  ...
  case FType of
    1: begin Canvas.Pen.Style := psDot; Canvas.Pen.Color := clBlack; Canvas.Pen.Width := 1; end;
    2: begin Canvas.Pen.Style := psDash; Canvas.Pen.Color := clBlack; Canvas.Pen.Width := 1; end;
    3: begin Canvas.Pen.Style := psSolid; Canvas.Pen.Color := clBlack; Canvas.Pen.Width := 1; end;
  end;
...
begin
  if (Message.Msg = WM_RBUTTONDOWN) or (Message.Msg = WM_LBUTTONDOWN) then
    if not PtInRegion(Rgn, TWMMouse(Message).XPos, TWMMouse(Message).YPos) then
      ControlAtPos(SmallPointToPoint(TWMMouse(Message).Pos)) else
...
procedure TOQBArea.InsertTable(X, Y: Integer; Link: Boolean);
var
  NewTable: TOQBTable;
  i, t, P1, P2: Integer;
  chave_1, chave_2: TStringList;
begin
  if FindTable(TOQBForm(GetParentForm(Self)).QBTables.Items[
    TOQBForm(GetParentForm(Self)).QBTables.ItemIndex]) <> nil then
  begin
    Mensagem('Tabela já inserida no diagrama!', modInformacao, [modOk]);
    Exit;
  end;
  TOQBForm(GetParentForm(Self)).FModify := True;
  ...
  try
    TabGlobal_i.DTABELAS.GotoBookmark(TOQBForm(GetParentForm(Self)).QBTables.Items.Objects[TOQBForm(GetParentForm(Self)).QBTables.ItemIndex]);
    NewTable.Activate(TOQBForm(GetParentForm(Self)).QBTables.Items[
      TOQBForm(GetParentForm(Self)).QBTables.ItemIndex], X, Y);
    size_X := NewTable.Width;
    if (NewTable.Height > size_Y) or (NewTable.Left <= 5) then
      size_Y := NewTable.Height;

    if Link then
    begin
      for i := ControlCount - 1 downto 0 do
        if Controls[i] is TOQBTable then
          UnlinkTable(TOQBTable(Controls[i]));

      with TOQBForm(GetParentForm(Self)).QBTables do
        for i := 0 to Items.Count-1 do
        begin
          if FindTable(Items[I]) <> Nil then
          begin
            TabGlobal_i.DTABELAS.GotoBookmark(Items.Objects[I]);
            TabGlobal_i.DRELACIONA.First;
            TabGlobal_i.DRELACIONA.Locate('NUMERO', TabGlobal_i.DTABELAS.NUMERO.Conteudo, []);
            while (TabGlobal_i.DRELACIONA.NUMERO.Conteudo = TabGlobal_i.DTABELAS.NUMERO.Conteudo) and
                  (not TabGlobal_i.DRELACIONA.Eof) do
            begin
              if (TabGlobal_i.DRELACIONA.ATIVO.Conteudo = 1) and (FindTable(TabGlobal_i.DRELACIONA.TAB_ESTRANGEIRA.Conteudo) <> Nil) then
              begin
                try
                  Chave_1 := TStringList.Create;
                  Chave_2 := TStringList.Create;
                  StringToArray(TabGlobal_i.DRELACIONA.CAMPOS_CHAVE_1.Conteudo, ';', Chave_1);
                  StringToArray(TabGlobal_i.DRELACIONA.CAMPOS_CHAVE_2.Conteudo, ';', Chave_2);
                  if Chave_1.Count = Chave_2.Count then
                    for t:=0 to Chave_1.Count-1 do
                    begin
                      P1 := FindTable(Items[I]).FLbx.Items.IndexOf(Chave_1[t]);
                      P2 := FindTable(TabGlobal_i.DRELACIONA.TAB_ESTRANGEIRA.Conteudo).FLbx.Items.IndexOf(Chave_2[t]);
                      if (P1 > -1) and (P2 > -1) then
                        InsertLink(FindTable(Items[I]), FindTable(TabGlobal_i.DRELACIONA.TAB_ESTRANGEIRA.Conteudo), P1, P2, TabGlobal_i.DRELACIONA.TIPO.Conteudo);
                    end;
                finally
                  Chave_1.Free;
                  Chave_2.Free;
...
function TOQBArea.InsertLink(_tbl1, _tbl2: TOQBTable;
  _fldN1, _fldN2, _type: Integer): TOQBLink;
begin
  ...
  begin
    //ShowMessage(Result.Tbl1.FTableName + ' :: ' + Result.Tbl1.FTableName + ^M + ^M +
    //            Result.FldNam1 + ' : ' + Result.FldNam2);
...
begin
  ...
  Link.Hint := Link.tbl1.FTableName + ' :: ' +
    Link.tbl2.FTableName;
  case Link.FType of
    1: Link.Hint := Link.Hint + ^M + 'Relacionamento de pesquisa';
    2: Link.Hint := Link.Hint + ^M + 'Integridade: Exclusão restrita';
    3: Link.Hint := Link.Hint + ^M + 'Integridade: Exclusão cascata';
  end;
  Link.ShowHint := True;
  with Link do
  begin
...
procedure TOQBForm.AutoFormatar;
var
  I, X, Y: Integer;
begin
  if Mensagem('Auto formatar?', modConfirmacao, [modSim, modNao]) = mrYes then
  begin
    FModify := True;
    X := 5;
    Y := 5;
    QBArea.size_Y := 0;
    for I:=0 to QBTables.Items.Count-1 do
    begin
      QBTables.ItemIndex := I;
      QBArea.InsertTable(X ,Y, IIF(I = QBTables.Items.Count-1, True, False));
      Inc(X, QBArea.size_X + 15);
      if X > QBArea.Width + 50 then
      begin
        X := 5;
        Inc(Y, QBArea.size_Y + 15);
      end;
    end;
  end;
end;
...
begin
  if Mensagem('Criar novo modelo?' + ^M + ^M + 'Atenção: Ao criar o novo modelo o atual será apagado!', modConfirmacao, [modSim, modNao]) = mrYes then
  begin
    FModify := True;
    ClearAll;
    AutoFormatar;
...
begin
  ...
  //  Exit;
  if Trim(QBDialog.OQBEngine.SQL.Text) = '' then
    exit;
  StrList := TStringList.Create;
  StrList.AddStrings(QBDialog.OQBEngine.SQL);
  ...
  try
    ...
    if j <> -1 then
      ...
      begin
        ...
        GetNextVal(ss2);
        ss2 := GetNextVal(ss2);
        NewLink := QBArea.InsertLink(Table1, Table2, FieldN1, FieldN2, StrToIntDef(ss2, 0));
        ss := GetNextVal(s);
...
begin
  ...
  QBDialog.OQBEngine.SQL.Clear;
  QBDialog.OQBEngine.SQL.AddStrings(StrList);
...
begin
  ...
  if not VSplitter.Visible then
    VSplitter.Tag := VSplitter.Left else
...
begin
  ...
  if not HSplitter.Visible then
    HSplitter.Tag := HSplitter.Top else
...
begin
  try
    ...
    for I:=0 to FormDB_Case.Tree_Tabelas.Items.Count-1 do
      if (FormDB_Case.Tree_Tabelas.Items[I].Level = 0) and
         (FormDB_Case.Tree_Tabelas.Items[I].ImageIndex <> 7) then
        QBTables.Items.AddObject(FormDB_Case.Tree_Tabelas.Items[I].Text, FormDB_Case.Tree_Tabelas.Items[I].Data);
...
begin
  if FModify then
  begin
    if Mensagem('Salvar diagrama?', modConfirmacao, [modSim, modNao]) = mrYes then
      btnSaveClick(Self)
    else
    begin
      FModify := False;
...
procedure TOQBForm.QBTablesDrawItem(Control: TWinControl; Index: Integer;
  ARect: TRect; State: TOwnerDrawState);
var
  r: TRect;
begin
...
procedure TOQBForm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if FModify then
    if Mensagem('Salvar diagrama?', modConfirmacao, [modSim, modNao]) = mrYes then
      btnSaveClick(Self);
...
procedure TOQBForm.Timer_AutoTimer(Sender: TObject);
begin
