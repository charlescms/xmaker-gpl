{
   Componente.: XDate.pas
   Copyright..: X-Maker - http://www.X-Makersoftware.com.br
   Data.......: 01/10/2003
}
unit XDate;
...
type
  TRangeCentury = 0..99;
  TXDateTime = Double;
  TXDateEdit = class(TCustomMaskEdit)
  private
    ...
    procedure SetDateValue(ADate: TXDateTime);
    ...
    function GetDateValue: TXDateTime;
  ...
  published
    ...
    property DateValue: TXDateTime read GetDateValue write SetDateValue;
    property DragCursor;
    property DragKind;
    property DragMode;
    property Enabled;
...
procedure TXDateEdit.CreateParams(var Params: TCreateParams);
const
...
procedure TXDateEdit.SetAlignment(Alig: TAlignment);
begin
...
procedure TXDateEdit.SetBaseChar(C: Char);
begin
...
procedure TXDateEdit.SetCentury(Bool: Boolean);
begin
...
procedure TXDateEdit.SetDateText(S: string);
begin
  FDate := GetDateInStr(FormatStrDate(S));
  DateChange;
end;

procedure TXDateEdit.SetDateValue(ADate: TXDateTime);
begin
  FDate := ADate;
  DateChange;
end;

function TXDateEdit.GetDateText: string;
begin
  Result := FormatDateTime(GetFormatMask, FDate)
end;

function TXDateEdit.GetDateValue: TXDateTime;
begin
  Result := FDate;
end;

procedure TXDateEdit.SetGlyph(Value: TBitmap);
begin
  if Value = nil then
...
procedure TXDateEdit.ChangeGlyph(Sender: TObject);
var
  NG: Byte;
begin
...
function TXDateEdit.GetEnabledBtn: Boolean;
begin
  Result := (FButton <> nil) and FButton.Enabled;
end;

procedure TXDateEdit.SetEnabledBtn(Value: Boolean);
begin
  if FButton <> nil then
    FButton.Enabled := Value;
end;

procedure TXDateEdit.SetHintBtn(Value: string);
begin
...
procedure TXDateEdit.SetShowButton(const Value: Boolean);
begin
  FShowButton := Value;
...
procedure TXDateEdit.DateChange;
begin
...
procedure TXDateEdit.CMEnter(var Msg: TCMEnter);
begin
...
procedure TXDateEdit.CMExit(var Msg: TCMExit);
var
  S: string;
begin
...
procedure TXDateEdit.WMMouse(var Msg: TWMMouse);
begin
  if not Focused then
    begin
      inherited;
...
procedure TXDateEdit.WMPaint(var Msg: TWMPaint);
var
  X: Integer;
  Rect: TRect;
  DC: HDC;
...
procedure TXDateEdit.WMSize(var Msg: TWMSize);
begin
  inherited;
  SetButtonSize;
  SetClientRect;
...
procedure TXDateEdit.WMSetFocus(var Msg: TMessage);
begin
  inherited;
  SetShowCaret;
end;

procedure TXDateEdit.SetShowCaret;
begin
  CreateCaret(Handle, 0, 1, GetCursorHeight);
  ShowCaret(Handle);
end;

function TXDateEdit.GetCursorHeight: Integer;
var
  DC: HDC;
  SaveFont: HFont;
  Metrics: TTextMetric;
...
function TXDateEdit.FormatStrDate(S: string): string;
var
  D, M, Y: Word;
...
function TXDateEdit.ExtractInvalidChar(S: string): string;
var
  I: Integer;
begin
  Result := '';
...
function TXDateEdit.GetMemberDate(S: string; Memb: Integer): string;
var
  I, F, M: Integer;
begin
  I := 1;
...
function TXDateEdit.GetDateInStr(S: string): TDateTime;
var
  Day, Month, Year: Word;
begin
...
function TXDateEdit.IsValidDate(S: string): Boolean;
var
  Day, Month, Year: Word;
begin
  DecodeStrDate(S, Year, Month, Day);
...
function TXDateEdit.GetFormatMask: string;
begin
...
function TXDateEdit.GetEditMask: string;
begin
...
function TXDateEdit.DaysInMonth(Month, Year: Integer): Integer;
const
...
function TXDateEdit.IsLeapYear(Year: Word): Boolean;
begin
  Result := (Year mod 4 = 0) and ((Year mod 100 <> 0) or (Year mod 400 = 0));
end;

function TXDateEdit.IsEmptyDate(S: string): Boolean;
var
  I: Integer;
  N: string;
begin
...
function TXDateEdit.AssumeDefaultDate(ADate: string): string;
var
...
procedure TXDateEdit.DecodeStrDate(ADate: string; var Year, Month, Day: Word);
var
  D, M, Y: Word;
begin
...
function TXDateEdit.Val(S: string): Integer;
begin
  S := NoSpaces(S);
  if S = '' then
    Result := 0
...
function TXDateEdit.NoSpaces(S: string): string;
var
  I: Integer;
begin
  Result := '';
...
procedure TXDateEdit.DateError;
begin
...
function TXDateEdit.GetTextMargins: TPoint;
var
  DC: HDC;
  SaveFont: HFont;
  I: Integer;
...
procedure TXDateEdit.CreateButton;
begin
...
procedure TXDateEdit.SetClientRect;
var
  Rect: TRect;
begin
  SendMessage(Handle, EM_GETRECT, 0, Longint(@Rect));
...
procedure TXDateEdit.SetButtonSize;
begin
  if FShowButton then
    begin
...
procedure TXDateEdit.ButtonClick(Sender: TObject);
begin
...
procedure TXDateEdit.ShowCalendar;
var
  Pos: TPoint;
begin
...
procedure Register;
begin
  RegisterComponents('X-Maker', [TXDateEdit]);
end;
