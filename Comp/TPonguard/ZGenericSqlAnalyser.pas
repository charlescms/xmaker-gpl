...
uses Classes, ZClasses, ZTokenizer, ZSelectSchema,
{$IFDEF VER130BELOW}
//  contnrs,
{$ENDIF}
  ZCompatibility;
...
begin
  ...
  begin
    if (Tokens.Count <= TokenIndex) then
      Break;
    if TZTokenType(Tokens.Objects[TokenIndex]) <> ttWord then
      Break;
    if Keyword <> '' then
      Keyword := Keyword + '*';
    ...
    begin
      if not (TZTokenType(Tokens.Objects[TokenIndex])
...
begin
  ...
  begin
    ...
    CurrentType := TZTokenType(SelectTokens.Objects[TokenIndex]);
...
begin
  ...
  begin
    ...
    CurrentUpper := AnsiUpperCase(CurrentValue);
    CurrentType := TZTokenType(FromTokens.Objects[TokenIndex]);
