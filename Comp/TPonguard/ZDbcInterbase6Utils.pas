...
type
  ...
  function GetVerion(PlainDriver: IZInterbasePlainDriver;
...
begin
  ...
  begin
    ...
    if Value < 0 then Temp := -0.001
    else Temp := 0.001;

    if (sqlscale < 0)  then
    begin
      case SQLCode of
        SQL_SHORT  : PSmallInt(sqldata)^ := Trunc(Value * IBScaleDivisor[sqlscale] + Temp);
        SQL_LONG   : PInteger(sqldata)^  := Trunc(Value * IBScaleDivisor[sqlscale] + Temp);
        SQL_INT64,
        SQL_QUAD   : PInt64(sqldata)^    := Trunc(Value * IBScaleDivisor[sqlscale] + Temp);
