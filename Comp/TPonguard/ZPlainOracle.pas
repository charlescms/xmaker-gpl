...
const
  {$IFNDEF LINUX}
  DEFAULT_DLL_LOCATION1   = 'OCI.DLL';
  DEFAULT_DLL_LOCATION2   = 'ORA803.DLL';
  {$ELSE}
  // the oracle 8 lib is: libclntsh.so.8.0
  DEFAULT_DLL_LOCATION1   = 'libclntsh.so';
  //DEFAULT_DLL_LOCATION2   = 'libwtc8.so';
  {$ENDIF}
...
{***************** Plain API constants definition ****************}
  ...
  cvOracle80000 = '8.0.0.0.0';
  cvOracle80400 = '8.0.4.0.0';
  cvOracle80500 = '8.0.5.0.0';
  cvOracle80501 = '8.0.5.0.1';
  cvOracle81000 = '8.1.0.0.0';
  cvOracle81500 = '8.1.5.0.0';
...
function OracleLoadLibrary: Boolean;

const
  DllLocation: string = DEFAULT_DLL_LOCATION1;
  DllHandle: THandle = 0;
  DllVersion: string = '';
...
begin
  if DllHandle = 0 then
  begin
    DllLocation := DEFAULT_DLL_LOCATION1;
    DllHandle := GetModuleHandle(PChar(DllLocation));
    DllLoaded := False;
    if DllHandle = 0 then
    begin
      DllHandle := LoadLibrary(PChar(DllLocation));
      DllLoaded := True;
    end;
  end;
  ...
  if DllHandle = 0 then
  begin
    DllLocation := DEFAULT_DLL_LOCATION2;
    DllHandle := GetModuleHandle(PChar(DllLocation));
    DllLoaded := False;
    if DllHandle = 0 then
    begin
  ...
  begin
{
    DllVersion := GetFileVersion(DllLocation);
...
//    if DllVersion >= cvOracle81000 then
      OCIEnvCreate := GetProcAddress(DllHandle, 'OCIEnvCreate');
//    else begin
      OCIInitialize := GetProcAddress(DllHandle, 'OCIInitialize');
      OCIEnvInit := GetProcAddress(DllHandle, 'OCIEnvInit');
//    end;

    OCIHandleAlloc := GetProcAddress(DllHandle, 'OCIHandleAlloc');
    OCIHandleFree := GetProcAddress(DllHandle, 'OCIHandleFree');
    OCIAttrSet := GetProcAddress(DllHandle, 'OCIAttrSet');
    OCIAttrGet := GetProcAddress(DllHandle, 'OCIAttrGet');
    OCIDescriptorAlloc := GetProcAddress(DllHandle, 'OCIDescriptorAlloc');
    OCIDescriptorFree := GetProcAddress(DllHandle, 'OCIDescriptorFree');
    OCIErrorGet := GetProcAddress(DllHandle, 'OCIErrorGet');

    OCIServerAttach := GetProcAddress(DllHandle, 'OCIServerAttach');
    OCIServerDetach := GetProcAddress(DllHandle, 'OCIServerDetach');
    OCIServerVersion := GetProcAddress(DllHandle, 'OCIServerVersion');
    OCIBreak := GetProcAddress(DllHandle, 'OCIBreak');
    if DLLVersion >= cvOracle81000 then
      OCIReset := GetProcAddress(DllHandle, 'OCIReset');

    OCISessionBegin := GetProcAddress(DllHandle, 'OCISessionBegin');
    OCISessionEnd := GetProcAddress(DllHandle, 'OCISessionEnd');
    OCIPasswordChange := GetProcAddress(DllHandle, 'OCIPasswordChange');

    OCITransStart := GetProcAddress(DllHandle, 'OCITransStart');
    OCITransCommit := GetProcAddress(DllHandle, 'OCITransCommit');
    OCITransRollback := GetProcAddress(DllHandle, 'OCITransRollback');
    OCITransDetach := GetProcAddress(DllHandle, 'OCITransDetach');
    OCITransPrepare := GetProcAddress(DllHandle, 'OCITransPrepare');
    OCITransForget := GetProcAddress(DllHandle, 'OCITransForget');

    OCIStmtPrepare := GetProcAddress(DllHandle, 'OCIStmtPrepare');
    OCIStmtExecute := GetProcAddress(DllHandle, 'OCIStmtExecute');
    OCIStmtFetch := GetProcAddress(DllHandle, 'OCIStmtFetch');
    OCIStmtGetPieceInfo := GetProcAddress(DllHandle, 'OCIStmtGetPieceInfo');
    OCIStmtSetPieceInfo := GetProcAddress(DllHandle, 'OCIStmtSetPieceInfo');
    OCIParamGet := GetProcAddress(DllHandle, 'OCIParamGet');
    OCIResultSetToStmt := GetProcAddress(DllHandle, 'OCIResultSetToStmt');

    OCIDefineByPos := GetProcAddress(DllHandle, 'OCIDefineByPos');
    OCIDefineArrayOfStruct := GetProcAddress(DllHandle, 'OCIDefineArrayOfStruct');

    OCIBindByPos := GetProcAddress(DllHandle, 'OCIBindByPos');
    OCIBindByName := GetProcAddress(DllHandle, 'OCIBindByName');
    OCIBindDynamic := GetProcAddress(DllHandle, 'OCIBindDynamic');

    OCILobAppend := GetProcAddress(DllHandle, 'OCILobAppend');
    OCILobAssign := GetProcAddress(DllHandle, 'OCILobAssign');
    OCILobCopy := GetProcAddress(DllHandle, 'OCILobCopy');
    OCILobEnableBuffering := GetProcAddress(DllHandle, 'OCILobEnableBuffering');
    OCILobDisableBuffering := GetProcAddress(DllHandle, 'OCILobDisableBuffering');
    OCILobErase := GetProcAddress(DllHandle, 'OCILobErase');
    OCILobFileExists := GetProcAddress(DllHandle, 'OCILobFileExists');
    OCILobFileGetName := GetProcAddress(DllHandle, 'OCILobFileGetName');
    OCILobFileSetName := GetProcAddress(DllHandle, 'OCILobFileSetName');
    OCILobFlushBuffer := GetProcAddress(DllHandle, 'OCILobFlushBuffer');
    OCILobGetLength := GetProcAddress(DllHandle, 'OCILobGetLength');
    OCILobLoadFromFile := GetProcAddress(DllHandle, 'OCILobLoadFromFile');
    OCILobLocatorIsInit := GetProcAddress(DllHandle, 'OCILobLocatorIsInit');
    OCILobRead := GetProcAddress(DllHandle, 'OCILobRead');
    OCILobTrim := GetProcAddress(DllHandle, 'OCILobTrim');
    OCILobWrite := GetProcAddress(DllHandle, 'OCILobWrite');
    if DLLVersion >= cvOracle81000 then
    begin
      OCILobClose := GetProcAddress(DllHandle, 'OCILobClose');
      OCILobIsOpen := GetProcAddress(DllHandle, 'OCILobIsOpen');
      OCILobOpen := GetProcAddress(DllHandle, 'OCILobOpen');
    end
    else
    begin
      OCILobClose := GetProcAddress(DllHandle, 'OCILobFileClose');
      OCILobIsOpen := GetProcAddress(DllHandle, 'OCILobFileIsOpen');
      OCILobOpen := GetProcAddress(DllHandle, 'OCILobFileOpen');
    end;

    OCIDescribeAny := GetProcAddress(DllHandle, 'OCIDescribeAny');

    Result := True;
  end
  else
    raise Exception.Create(Format(SLibraryNotFound,[DllLocation]));
end;
...
finalization
  if (DllHandle <> 0) and DllLoaded then
