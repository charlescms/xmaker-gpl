...
{$ENDIF}
  ...
  Classes, SysUtils, Windows, Menus, ZConnection, ZAbstractDataset,
{$IFDEF LINUX}
...
uses Dialogs, LibHelp, TypInfo, ZSqlMetadata,
...
begin
  ...
  if Assigned(DataSet) and Assigned(DataSet.Connection) and Assigned(DataSet.Connection.dbcConnection)then
  begin
    QuoteChar := DataSet.Connection.dbcConnection.GetMetadata.GetIdentifierQuoteString;
    if Length(QuoteChar) = 1 then
      QuoteChar := QuoteChar + QuoteChar;
    { Parses the Select statement and retrieves a schema object. }
    Tokenizer := DataSet.Connection.dbcConnection.GetMetadata.GetTokenizer;
    StatementAnalyser := DataSet.Connection.dbcConnection.GetMetadata.GetStatementAnalyser;
    SelectSchema := StatementAnalyser.DefineSelectSchemaFromQuery(Tokenizer, THackDataSet(DataSet).SQL.Text);
