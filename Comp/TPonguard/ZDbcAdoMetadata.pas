...
{$ENDIF}
  ...
  {$IFNDEF VER125BELOW}OleDB, {$ENDIF} ZDbcAdoResultSet;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Result.UpdateShortByName('PROCEDURE_TYPE', GetShortByName('PROCEDURE_TYPE') - 1);
        Result.InsertRow;
      end;
      Close;
      Free;
...
begin
  ...
  AdoRecordSet := AdoOpenSchema(adSchemaProcedureParameters, [Catalog, SchemaPattern, ProcedureNamePattern]);
  if Assigned(AdoRecordSet) then
    with TZAdoResultSet.Create(GetStatement, '', AdoRecordSet) do
    begin
      while Next do
...
begin
  ...
  if GetConnection.IsClosed then GetConnection.Open;

  AdoRecordSet := AdoOpenSchema(adSchemaTables, [Catalog, SchemaPattern, TableNamePattern, TableTypes]);
  if Assigned(AdoRecordSet) then
    with TZAdoResultSet.Create(GetStatement, '', AdoRecordset) do
    begin
      while Next do
      begin
        ...
        Result.UpdateStringByName('TABLE_SCHEM', GetStringByName('TABLE_SCHEMA'));
...
begin
  ...
  if Assigned(AdoRecordSet) then
    ...
    begin
      ...
      begin
        Result.MoveToInsertRow;
        Result.UpdateStringByName('TABLE_SCHEM', GetStringByName('SCHEMA_NAME'));
        Result.InsertRow;
      end;
      Close;
      Free;
...
begin
  ...
  if Assigned(AdoRecordSet) then
    ...
    begin
      ...
      begin
        Result.MoveToInsertRow;
        Result.UpdateStringByName('TABLE_CAT', GetStringByName('CATALOG_NAME'));
        Result.InsertRow;
      end;
      Close;
      Free;
...
const
  TableTypes: array[0..7] of string = ('ALIAS', 'TABLE', 'SYNONYM', 'SYSTEM TABLE', 'VIEW', 'GLOBAL TEMPORARY', 'LOCAL TEMPORARY', 'SYSTEM VIEW');
var
  I: Integer;
begin
...
begin
  ...
  AdoRecordSet := AdoOpenSchema(adSchemaColumns, [Catalog, SchemaPattern, TableNamePattern, ColumnNamePattern]);
  ...
  begin
    ...
    begin
      ...
      begin
        Result.MoveToInsertRow;
        Result.UpdateStringByName('TABLE_CAT', ''{GetStringByName('TABLE_CATALOG')});
        Result.UpdateStringByName('TABLE_SCHEM', ''{GetStringByName('TABLE_SCHEMA')});
        Result.UpdateStringByName('TABLE_NAME', GetStringByName('TABLE_NAME'));
...
//            Result.UpdateShortByName('DATA_TYPE', Ord(stUnicodeStream));
        Result.UpdateIntByName('COLUMN_SIZE', GetIntByName('CHARACTER_MAXIMUM_LENGTH'));
        Result.UpdateIntByName('BUFFER_LENGTH', GetIntByName('CHARACTER_MAXIMUM_LENGTH'));
        Result.UpdateIntByName('DECIMAL_DIGITS', GetIntByName('NUMERIC_SCALE'));
        Result.UpdateIntByName('NUM_PREC_RADIX', GetShortByName('NUMERIC_PRECISION'));
        if GetBooleanByName('IS_NULLABLE') then
          Result.UpdateShortByName('NULLABLE', 1)
        else
          Result.UpdateShortByName('NULLABLE', 0);
        Result.UpdateStringByName('REMARKS', GetStringByName('DESCRIPTION'));
        Result.UpdateStringByName('COLUMN_DEF', GetStringByName('COLUMN_DEFAULT'));
...
begin
  ...
  AdoRecordSet := AdoOpenSchema(adSchemaColumnPrivileges, [Catalog, Schema, Table, ColumnNamePattern]);
  if Assigned(AdoRecordSet) then
    with TZAdoResultSet.Create(GetStatement, '', AdoRecordSet) do
    begin
      while Next do
      begin
        ...
        Result.UpdateStringByName('PRIVILEGE', GetStringByName('PRIVILEGE_TYPE'));
...
begin
  ...
  AdoRecordSet := AdoOpenSchema(adSchemaTablePrivileges, [Catalog, SchemaPattern, TableNamePattern]);
  if Assigned(AdoRecordSet) then
    with TZAdoResultSet.Create(GetStatement, '', AdoRecordSet) do
    begin
      while Next do
...
function TZAdoDatabaseMetadata.GetBestRowIdentifier(Catalog: string;
...
begin
  ...
  if Assigned(AdoRecordSet) then
    ...
    begin
      while Next do
      begin
        if (GetIntByName('COLUMN_FLAGS') and DBCOLUMNFLAGS_ISROWVER) = 0 then
          Continue;
        Result.MoveToInsertRow;
        ...
        Result.UpdateShortByName('DATA_TYPE', Ord(ConvertAdoToSqlType(GetShortByName('DATA_TYPE'))));
        Result.UpdateStringByName('TYPE_NAME', GetStringByName('TYPE_NAME'));
        Result.UpdateIntByName('COLUMN_SIZE', GetIntByName('CHARACTER_OCTET_LENGTH'));
        Result.UpdateIntByName('BUFFER_LENGTH', GetIntByName('CHARACTER_OCTET_LENGTH'));
        Result.UpdateIntByName('DECIMAL_DIGITS', GetIntByName('NUMERIC_SCALE'));
...
begin
  ...
  AdoRecordSet := AdoOpenSchema(adSchemaPrimaryKeys, [Catalog, Schema, Table]);
  if Assigned(AdoRecordSet) then
    with TZAdoResultSet.Create(GetStatement, '', AdoRecordSet) do
    begin
      while Next do
      begin
        ...
        Result.UpdateStringByName('TABLE_NAME', GetStringByName('TABLE_NAME'));
        Result.UpdateStringByName('COLUMN_NAME', GetStringByName('COLUMN_NAME'));
        Result.UpdateShortByName('KEY_SEQ', GetShortByName('ORDINAL'));
...
begin
  ...
  AdoRecordSet := AdoOpenSchema(adSchemaForeignKeys, [PrimaryCatalog, PrimarySchema, PrimaryTable, ForeignCatalog, ForeignSchema, ForeignTable]);
  if Assigned(AdoRecordSet) then
    with TZAdoResultSet.Create(GetStatement, '', AdoRecordSet) do
    begin
      while Next do
  ...
  //!!!      Result.UpdateShortByName('DELETE_RULE', GetShortByName('DELETE_RULE'));
        ...
        Result.UpdateStringByName('PK_NAME', GetStringByName('PK_NAME'));
        Result.UpdateIntByName('DEFERRABILITY', GetShortByName('DEFERRABILITY'));
        Result.InsertRow;
      end;
      Close;
      Free;
...
begin
  ...
  AdoRecordSet := AdoOpenSchema(adSchemaIndexes, [Catalog, Schema, '', '', Table]);
  if Assigned(AdoRecordSet) then
    with TZAdoResultSet.Create(GetStatement, '', AdoRecordSet) do
    begin
      while Next do
...
begin
  ...
  try
    ...
    Result := (GetConnection as IZAdoConnection).GetAdoConnection.OpenSchema(Schema, Restrictions, EmptyParam);
  except
    Result := nil;
  end;
end;
