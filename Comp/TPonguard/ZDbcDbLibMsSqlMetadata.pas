...
begin
  ...
  if Catalog = '' then Catalog := 'null' else Catalog := AnsiQuotedStr(Catalog, '''');
  if SchemaPattern = '' then SchemaPattern := 'null' else SchemaPattern := AnsiQuotedStr(SchemaPattern, '''');
  if ProcedureNamePattern = '' then ProcedureNamePattern := 'null' else ProcedureNamePattern := AnsiQuotedStr(ProcedureNamePattern, '''');

  with GetStatement.ExecuteQuery(Format('exec sp_stored_procedures %s, %s, %s', [ProcedureNamePattern, SchemaPattern, Catalog])) do
  begin
    while Next do
    begin
...
begin
  ...
  if SchemaPattern = '' then SchemaPattern := 'null' else SchemaPattern := AnsiQuotedStr(SchemaPattern, '''');
  if ProcedureNamePattern = '' then ProcedureNamePattern := 'null' else ProcedureNamePattern := AnsiQuotedStr(ProcedureNamePattern, '''');
  if ColumnNamePattern = '' then ColumnNamePattern := 'null' else ColumnNamePattern := AnsiQuotedStr(ColumnNamePattern, '''');

  with GetStatement.ExecuteQuery(Format('exec sp_sproc_columns %s, %s, %s, %s', [ProcedureNamePattern, SchemaPattern, Catalog, ColumnNamePattern])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  if Catalog = '' then Catalog := 'null' else Catalog := AnsiQuotedStr(Catalog, '''');
  if SchemaPattern = '' then SchemaPattern := 'null' else SchemaPattern := AnsiQuotedStr(SchemaPattern, '''');
  if TableNamePattern = '' then TableNamePattern := 'null' else TableNamePattern := AnsiQuotedStr(TableNamePattern, '''');
  TableTypes := '';
  ...
  with GetStatement.ExecuteQuery(Format('exec sp_tables %s, %s, %s, %s', [TableNamePattern, SchemaPattern, Catalog, TableTypes])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  with GetStatement.ExecuteQuery('select name as TABLE_OWNER from sysusers where islogin = 1') do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  begin
    ...
    begin
      Result.MoveToInsertRow;
      Result.UpdateStringByName('TABLE_CAT', GetStringByName('DATABASE_NAME'));
      Result.InsertRow;
    end;
    Close;
  end;
...
begin
  ...
  with GetStatement.ExecuteQuery(Format('exec sp_columns %s, %s, %s, %s', [TableNamePattern, SchemaPattern, Catalog, ColumnNamePattern])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
//        Ord(ConvertODBCToSqlType(GetShortByName('DATA_TYPE'))));
      ...
      Result.UpdateBooleanByName('SEARCHABLE', not (GetShortByName('SS_DATA_TYPE') in [34, 35]));
  ...
  Result.BeforeFirst;
  with GetStatement.ExecuteQuery(Format('select c.colid, c.name, c.type, c.prec, c.scale, c.colstat, c.status, c.iscomputed from syscolumns c inner join sysobjects o on (o.id = c.id) where o.name = %s order by colid', [TableNamePattern])) do
  begin
    while Next do
    begin
      Result.Next;
      Result.UpdateBooleanByName('AUTO_INCREMENT', (GetShortByName('status') and $80) <> 0);
      Result.UpdateNullByName('CASE_SENSITIVE');
      Result.UpdateBooleanByName('SEARCHABLE', Result.GetBooleanByName('SEARCHABLE') and (GetIntByName('iscomputed') = 0));
      Result.UpdateBooleanByName('WRITABLE', ((GetShortByName('status') and $80) = 0) and (GetShortByName('type') <> 37) and (GetIntByName('iscomputed') = 0));
      Result.UpdateBooleanByName('DEFINITELYWRITABLE', Result.GetBooleanByName('WRITABLE'));
      Result.UpdateBooleanByName('READONLY', not Result.GetBooleanByName('WRITABLE'));
...
begin
  ...
  if Catalog = '' then Catalog := 'null' else Catalog := AnsiQuotedStr(Catalog, '''');
  if Schema = '' then Schema := 'null' else Schema := AnsiQuotedStr(Schema, '''');
  if Table = '' then Table := 'null' else Table := AnsiQuotedStr(Table, '''');
  if ColumnNamePattern = '' then ColumnNamePattern := 'null' else ColumnNamePattern := AnsiQuotedStr(ColumnNamePattern, '''');

  with GetStatement.ExecuteQuery(Format('exec sp_column_privileges %s, %s, %s, %s', [Table, Schema, Catalog, ColumnNamePattern])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  with GetStatement.ExecuteQuery(Format('exec sp_table_privileges %s, %s, %s', [TableNamePattern, SchemaPattern, Catalog])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
function TZMsSqlDatabaseMetadata.GetBestRowIdentifier(Catalog: string;
  Schema: string; Table: string; Scope: Integer; Nullable: Boolean): IZResultSet;
var
  MSCol_Type, MSScope, MSNullable: string;
begin
  ...
  if Table = '' then Table := 'null' else Table := AnsiQuotedStr(Table, '''');
  MSCol_Type := '''R''';
  if Scope = 0 then MSScope := '''C''' else MSScope := '''T''';
  If Nullable then MSNullable := '''U''' else MSNullable := '''O''';

  with GetStatement.ExecuteQuery(Format('exec sp_special_columns %s, %s, %s, %s, %s, %s', [Table, Schema, Catalog, MSCol_Type, MSScope, MSNullable])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  with GetStatement.ExecuteQuery(Format('exec sp_special_columns %s, %s, %s, %s', [Table, Schema, Catalog, MSCol_Type])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  with GetStatement.ExecuteQuery(Format('exec sp_pkeys %s, %s, %s', [Table, Schema, Catalog])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  if PrimaryCatalog = '' then PrimaryCatalog := 'null' else PrimaryCatalog := AnsiQuotedStr(PrimaryCatalog, '''');
  if PrimarySchema = '' then PrimarySchema := 'null' else PrimarySchema := AnsiQuotedStr(PrimarySchema, '''');
  if PrimaryTable = '' then PrimaryTable := 'null' else PrimaryTable := AnsiQuotedStr(PrimaryTable, '''');
  if ForeignCatalog = '' then ForeignCatalog := 'null' else ForeignCatalog := AnsiQuotedStr(ForeignCatalog, '''');
  if ForeignSchema = '' then ForeignSchema := 'null' else ForeignSchema := AnsiQuotedStr(ForeignSchema, '''');
  if ForeignTable = '' then ForeignTable := 'null' else ForeignTable := AnsiQuotedStr(ForeignTable, '''');

  with GetStatement.ExecuteQuery(Format('exec sp_fkeys %s, %s, %s, %s, %s, %s', [PrimaryTable, PrimarySchema, PrimaryCatalog, ForeignTable, ForeignSchema, ForeignCatalog])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  if Schema = '' then Schema := 'null' else Schema := AnsiQuotedStr(Schema, '''');
  if Table = '' then Table := 'null' else Table := AnsiQuotedStr(Table, '''');
  if Unique then Is_Unique := '''Y''' else Is_Unique := '''N''';
  if Approximate then Accuracy := '''Q''' else Accuracy := '''E''';

  with GetStatement.ExecuteQuery(Format('exec sp_statistics %s, %s, %s, ''%%'', %s, %s', [Table, Schema, Catalog, Is_Unique, Accuracy])) do
  begin
    while Next do
    begin
      Result.MoveToInsertRow;
...
begin
  ...
  if SchemaPattern = '' then SchemaPattern := '''%''' else SchemaPattern := AnsiQuotedStr(SchemaPattern, '''');
  if TypeNamePattern = '' then TypeNamePattern := '''%''' else TypeNamePattern := AnsiQuotedStr(TypeNamePattern, '''');
...
function TZMsSqlDatabaseMetadata.GetTokenizer: IZTokenizer;
begin
...
function TZMsSqlDatabaseMetadata.GetStatementAnalyser: IZStatementAnalyser;
begin
