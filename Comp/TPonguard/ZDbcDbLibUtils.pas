...
{$IFDEF VER125Below}
      stTime:
      begin
        DecodeTime(SoftVarManager.GetAsDateTime(Value), Hour, Min, Sec, MSec);
        Result := '''' + FormatDateTime('hh":"mm":"ss',
          SoftVarManager.GetAsDateTime(Value)) + FormatFloat('":"000', MSec) + '''' ;
      end;
      stTimestamp:
      begin
        DecodeTime(SoftVarManager.GetAsDateTime(Value), Hour, Min, Sec, MSec);
        Result := '''' + FormatDateTime('yyyymmdd hh":"mm":"ss',
          SoftVarManager.GetAsDateTime(Value)) + FormatFloat('":"000', MSec) + '''';
      end;
{$ELSE}
      stTime:
