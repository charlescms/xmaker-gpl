...
begin
  ...
  TypeName := Connection.GetTypeNameByOid(TypeOid);

  SQLType :=  PostgreSQLToSQLType(Connection, TypeName);
  ...
  else if TypeName = 'interval' then
    ColumnInfo.Precision := 32
  else if TypeName = 'regproc' then
    ColumnInfo.Precision := 10
  else if (TypeName = 'bytea') and Connection.IsOidAsBlob then
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (Precision = 0) or (ColumnLabel = 'expr') then
          Precision := 255;
      end;
    end;
...
begin
  ...
  CheckPostgreSQLError(nil, FPlainDriver, FHandle, lcOther, 'Open Large Object');
