...
begin
  ...
  if Catalog = '' then Catalog := 'null' else Catalog := AnsiQuotedStr(Catalog, '''');
  if SchemaPattern = '' then SchemaPattern := 'null' else SchemaPattern := AnsiQuotedStr(SchemaPattern, '''');
  ProcNamePart := ProcedureNamePattern;
  if ProcedureNamePattern = '' then ProcedureNamePattern := 'null' else ProcedureNamePattern := AnsiQuotedStr(ProcedureNamePattern, '''');
  ...
  begin
    NumberPart := Copy(ProcNamePart, LastDelimiter(';', ProcNamePart) + 1, Length(ProcNamePart));
    if NumberPart = '' then
      NumberPart := '1';
...
//status2 is added in sybase ASE 12.5 to store the storedprocedure parameters input/output type
//this column does not exists in prior versions. In prior versions there is no way to determine between input or output type.
  with GetStatement.ExecuteQuery(Format('select c.* from syscolumns c inner join sysobjects o on (o.id = c.id) where o.name = %s and c.number = %s order by colid', [AnsiQuotedStr(ProcNamePart, ''''), NumberPart])) do
  begin
...
//        Ord(ConvertODBCToSqlType(GetShortByName('DATA_TYPE'))));
  ...
  Result.BeforeFirst;
  with GetStatement.ExecuteQuery(Format('select c.colid, c.name, c.type, c.prec, c.scale, c.status from syscolumns c inner join sysobjects o on (o.id = c.id) where o.name = %s order by colid', [TableNamePattern])) do
  begin
    while Next do
    begin
      Result.Next;
      ...
      Result.UpdateBooleanByName('SEARCHABLE', not (GetShortByName('type') in [34, 35]));
      Result.UpdateBooleanByName('WRITABLE', ((GetShortByName('status') and $80) = 0) and (GetShortByName('type') <> 37));
...
begin
  ...
  if ColumnNamePattern = '' then ColumnNamePattern := '''%''' else ColumnNamePattern := AnsiQuotedStr(ColumnNamePattern, '''');
