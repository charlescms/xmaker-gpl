...
uses ZCompatibility, Classes, SysUtils, ZDbcIntfs, ZDbcConnection,
  ZPlainPostgreSqlDriver, ZDbcLogging;
...
type
  ...
  public
    constructor Create(Url: string; PlainDriver: IZPostgreSQLPlainDriver;
      HostName: string; Port: Integer; Database: string; User: string;
      Password: string; Info: TStrings);
...
uses
  ...
  ZDbcPostgreSqlUtils, ZDbcPostgreSqlMetadata;
...
begin
  ...
  try
    ...
    Result := TZPostgreSQLConnection.Create(Url, PlainDriver, HostName, Port,
...
begin
  ...
  Result[3] := FPostgreSQL73PlainDriver.GetProtocol;
end;
...
begin
  ...
  else if Protocol = FPostgreSQL73PlainDriver.GetProtocol then
    Result := FPostgreSQL73PlainDriver
  else Result := FPostgreSQL73PlainDriver;
  Result.Initialize;
end;
...
constructor TZPostgreSQLConnection.Create(Url: string;
...
begin
  ...
  SQL := 'SELECT version(), CASE WHEN pg_encoding_to_char(1)=''SQL_ASCII'''
