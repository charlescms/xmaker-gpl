...
type
  ...
  IZMySQLPlainDriver = interface (IZPlainDriver)
    ...
    function GetAffectedRows(Handle: PZMySQLConnect): Cardinal;
...
uses SysUtils, ZPlainMySql320, ZPlainMySql323, ZPlainMySql40;
...
begin
  Result := ZPlainMySql320.mysql_real_query(Handle, Query);
end;
...
function TZMySQL320PlainDriver.GetAffectedRows(
  Handle: PZMySQLConnect): Cardinal;
begin
...
function TZMySQL323PlainDriver.ExecRealQuery(Handle: PZMySQLConnect;
  const Query: PChar; Length: Integer): Integer;
begin
  Result := ZPlainMySql323.mysql_real_query(Handle, Query);
end;
...
function TZMySQL40PlainDriver.ExecRealQuery(Handle: PZMySQLConnect;
  const Query: PChar; Length: Integer): Integer;
begin
  Result := ZPlainMySql40.mysql_real_query(Handle, Query);
end;
...
function TZMySQL40PlainDriver.GetAffectedRows(Handle: PZMySQLConnect): Cardinal;
begin
