...
uses
  ...
  Menus, StdCtrls, Buttons, ExtCtrls, Clipbrd;

type
  TXNumEdit = class(TCustomEdit)
  private
...
constructor TXNumEdit.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FCanvas := TControlCanvas.Create;
  FCanvas.Control := Self;
...
procedure TXNumEdit.CreateParams(var Params: TCreateParams);
const
...
procedure TXNumEdit.SetZeroLength(Value: Byte);
begin
...
procedure TXNumEdit.ConvertText;
begin
...
function TXNumEdit.StrZero(Number: string; Len: Integer): string;
begin
...
function TXNumEdit.ExtractInvalidChar(S: string): string;
var
  I: Integer;
begin
  Result := '';
...
procedure TXNumEdit.CreateWnd;
begin
  inherited CreateWnd;
  SetClientRect;
end;

procedure TXNumEdit.SetAlignment(Alig: TAlignment);
begin
...
procedure TXNumEdit.SetDecimals(Dec: Byte);
begin
  FDecimals := Dec;
...
procedure TXNumEdit.SetEnabledBtn(Value: Boolean);
begin
  if FButton <> nil then
    FButton.Enabled := Value;
end;

procedure TXNumEdit.SetGlyph(Value: TBitmap);
begin
  if Value = nil then
...
procedure TXNumEdit.SetHintBtn(Value: string);
begin
...
procedure TXNumEdit.SetShowButton(const Value: Boolean);
begin
  FShowButton := Value;
...
procedure TXNumEdit.SetShowSeparator(Value: Boolean);
begin
...
procedure TXNumEdit.SetValue(Val: Extended);
begin
...
function TXNumEdit.StrToValue(S: string): Extended;
var
  N: string;
  I: Integer;
begin
...
procedure TXNumEdit.ChangeGlyph(Sender: TObject);
var
  NG: Byte;
begin
...
procedure TXNumEdit.CMEnter(var Msg: TCMEnter);
begin
  if FZeroLength > 0 then
    Text := StrZero(FloatToStr(GetValue), FZeroLength)
  else
...
procedure TXNumEdit.CMExit(var Msg: TCMExit);
begin
  if FZeroLength > 0 then
    Text := StrZero(FloatToStr(GetValue), FZeroLength)
  else
...
procedure TXNumEdit.WMMouse(var Msg: TWMMouse);
begin
  if not Focused then
    begin
      inherited;
...
procedure TXNumEdit.WMPaint(var Msg: TWMPaint);
var
  X: Integer;
  Rect: TRect;
  DC: HDC;
...
procedure TXNumEdit.WMSize(var Msg: TWMSize);
begin
  inherited;
  SetButtonSize;
  SetClientRect;
...
procedure TXNumEdit.WMPaste(var Msg: TMessage);
var
  S: string;
  I: Integer;
  Key: Char;
...
procedure TXNumEdit.WMSetFocus(var Msg: TMessage);
begin
  inherited;
  SetShowCaret;
end;

procedure TXNumEdit.SetShowCaret;
begin
  CreateCaret(Handle, 0, 1, GetCursorHeight);
  ShowCaret(Handle);
end;

function TXNumEdit.GetCursorHeight: Integer;
var
  DC: HDC;
  SaveFont: HFont;
  Metrics: TTextMetric;
...
procedure TXNumEdit.KeyPress(var Key: Char);
var
  Form: TCustomForm;
begin
...
procedure TXNumEdit.ValidateKey;
begin
  if FKey in ['.', ','] then
...
procedure TXNumEdit.InvertValue;
var
  I, L: Integer;
begin
  L := Length(Text);
...
procedure TXNumEdit.EvalDecSeparator;
var
  I, L: Integer;
begin
  L := Length(Text);
...
procedure TXNumEdit.EvaluateKey;
var
  I, L: Integer;
begin
  L := Length(Text);
...
function TXNumEdit.CheckRange: Boolean;
begin
  Result := True;
...
function TXNumEdit.GetTextMargins: TPoint;
var
  DC: HDC;
  SaveFont: HFont;
  I: Integer;
...
procedure TXNumEdit.CreateButton;
begin
...
procedure TXNumEdit.SetClientRect;
var
  Rect: TRect;
begin
  SendMessage(Handle, EM_GETRECT, 0, Longint(@Rect));
...
procedure TXNumEdit.SetButtonSize;
begin
  if FShowButton then
    begin
...
procedure TXNumEdit.CheckLimitText;
var
  Margins: TPoint;
  I: Integer;
begin
...
procedure TXNumEdit.ButtonClick(Sender: TObject);
begin
...
procedure TXNumEdit.ShowCalculator;
var
  Pos: TPoint;
begin
  if FrmXNumEdit = nil then
    Application.CreateForm(TFrmXNumEdit, FrmXNumEdit);
  Pos := Point(0, 0);
  Pos := Self.ClientToScreen(Pos);
  if (Pos.X + 135) > Screen.DesktopWidth then
    FrmXNumEdit.Left := Pos.X - (FrmXNumEdit.Width - Width) - 2
  else
    FrmXNumEdit.Left := Pos.X - 2;
  if (Pos.Y + 205) > Screen.DesktopHeight then
    FrmXNumEdit.Top := Pos.Y - FrmXNumEdit.Height - 2
  else
    FrmXNumEdit.Top := Pos.Y + Height - 3;
  FrmXNumEdit.FOnwerEdt := Self;
  FrmXNumEdit.ResetCalc;
  FrmXNumEdit.FValBuffer := GetValue;
  FrmXNumEdit.PanDisplay.Caption := FloatToStr(GetValue);
  FrmXNumEdit.Show;
end;


procedure TFrmXNumEdit.FormCreate(Sender: TObject);
begin
  ResetCalc;
end;

procedure TFrmXNumEdit.FormDeactivate(Sender: TObject);
begin
  if FOnwerEdt.FAutoHideCalculator then
    Close;
end;

procedure TFrmXNumEdit.FormKeyPress(Sender: TObject; var Key: Char);
begin
  EvalKey(Key);
end;

procedure TFrmXNumEdit.BtnNumClick(Sender: TObject);
begin
  EvalKey(TSpeedButton(Sender).Caption[1]);
end;

procedure TFrmXNumEdit.EvalKey(Key: Char);
begin
  if Key in [',', '.', '·'] then
...
procedure TFrmXNumEdit.SendKeyToDiplay(Key: Char);
begin
...
procedure TFrmXNumEdit.ResetCalc;
begin
  FValBuffer := 0;
  FLastEnter := '';
  FOperator := #0;
...
procedure TFrmXNumEdit.BtnCEClick(Sender: TObject);
begin
  FLastEnter := '';
  PanDisplay.Caption := '0';
end;

procedure TFrmXNumEdit.BtnCClick(Sender: TObject);
begin
  ResetCalc;
  PanDisplay.Caption := '0';
end;

function TFrmXNumEdit.GetDisplayValue: Double;
begin
  if FLastEnter <> '' then
...
procedure TFrmXNumEdit.BtnBaseOpClick(Sender: TObject);
begin
...
procedure TFrmXNumEdit.CalcOperation;
var
  X: Double;
begin
  X := GetDisplayValue;
...
procedure TFrmXNumEdit.BtnResClick(Sender: TObject);
begin
  if FEnterChanged and (FOperator <> #0) then
    begin
      CalcOperation;
...
procedure TFrmXNumEdit.ShowResult;
var
  S: string;
begin
...
procedure TFrmXNumEdit.BtnSgnClick(Sender: TObject);
begin
...
procedure TFrmXNumEdit.BtnPercClick(Sender: TObject);
var
  X: Double;
  S: string;
begin
...
procedure TFrmXNumEdit.BtnCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TFrmXNumEdit.BtnOKClick(Sender: TObject);
begin
  if PanDisplay.Caption <> 'E' then
    begin
...
procedure TFrmXNumEdit.PanMouseDown(Sender: TObject;
...
procedure TFrmXNumEdit.PanMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure Register;
begin
  RegisterComponents('X-Maker', [TXNumEdit]);
end;
