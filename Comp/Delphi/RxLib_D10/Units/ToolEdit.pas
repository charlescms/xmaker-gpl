...
{$ENDIF RX_D4}
    ...
    try
      ...
      with ACanvas do begin
        ...
        AWidth := TextWidth(S);
        {$IFDEF VER180}
        Margins.Left := EditorTextMargins(Editor).X;
        Margins.Top := EditorTextMargins(Editor).Y;
        {$ELSE}
        Margins := EditorTextMargins(Editor);
        {$ENDIF}

        {$IFDEF VER180}
        if PopupVisible then ALeft := Margins.Left
        {$ELSE}
        ...
        {$ENDIF}
          ...
          case AAlignment of
            ...
            taRightJustify:
              {$IFDEF VER180}
              ALeft := ClientWidth - ButtonWidth - AWidth - Margins.Left - 2;
              {$ELSE}
...
{$ENDIF}
        ...
        {$IFDEF VER180}
        TextRect(R, ALeft, Margins.Top, S);
        {$ELSE}
