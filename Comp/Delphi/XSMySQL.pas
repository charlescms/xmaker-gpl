{$I XSqlDir.inc}
unit XSMySql {$IFDEF XSQL_CLR} platform {$ENDIF};
...
begin
 Result := {$IFDEF XSQL_D4} FAutoIncID {$ELSE} Trunc( FAutoIncID ) {$ENDIF};
end;

function TIMyCommand.GetRowsAffected: Integer;
begin
  Result := {$IFDEF XSQL_D4} FRowsAffected {$ELSE} Trunc( FRowsAffected ) {$ENDIF};
end;
...
begin
  Result := FieldType in
  	[ftInteger, {$IFDEF XSQL_VCL4}ftLargeInt,{$ENDIF} ftSmallInt, ftAutoInc,
         ftFloat, ftCurrency,
