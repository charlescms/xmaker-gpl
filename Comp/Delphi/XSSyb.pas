{$I XSqlDir.inc}
unit XSSyb {$IFDEF XSQL_CLR} platform {$ENDIF};
...
*}
  ...
  TCS_OID	= _cs_oid;
  PCS_OID	= {$IFDEF XSQL_CLR}TCS_VOID{$ELSE}^TCS_OID{$ENDIF};
...
{ TISybDatabase }
  ...
  private
    FHandle: TSDPtr;
    FIsSingleConn: Boolean;    		// Whether or not a database FHandle is used for opened TXSQLQuery
...
begin
  errcode := -1;
  errmsg  := '';
  if (ct_diag( conn, CS_STATUS, CS_SERVERMSG_TYPE, CS_UNUSED, {$IFNDEF XSQL_CLR}@{$ENDIF}smsgnum ) = CS_SUCCEED)
     and (smsgnum > 0)
  then begin
    for erridx := 1 to smsgnum do begin
      ct_diag( conn, CS_GET, CS_SERVERMSG_TYPE, erridx, {$IFNDEF XSQL_CLR}@{$ENDIF}smsg );
  ...
  if (ct_diag( conn, CS_STATUS, CS_CLIENTMSG_TYPE, CS_UNUSED, {$IFNDEF XSQL_CLR}@{$ENDIF}cmsgnum ) = CS_SUCCEED) and
     (cmsgnum > 0)
  then begin
    for errIdx := 1 to cmsgnum do begin
      ct_diag( conn, CS_GET, CS_CLIENTMSG_TYPE, 1, {$IFNDEF XSQL_CLR}@{$ENDIF}cmsg );
...
begin
  Result := 0;
  ct_config( ctx, CS_GET, prop, {$IFDEF XSQL_CLR}Result{$ELSE}PCS_VOID(@Result){$ENDIF}, CS_UNUSED, nil );
end;

procedure SybCtxSetPropInt32(ctx: PCS_CONTEXT; prop, Value: TCS_INT);
begin
  ct_config( ctx, CS_SET, prop, {$IFDEF XSQL_CLR}Value{$ELSE}PCS_VOID(@Value){$ENDIF}, CS_UNUSED, nil );
end;

function SybConGetOptionInt32(con: PCS_CONNECTION; option: TCS_INT; var Value: TCS_INT): TCS_RETCODE;
begin
  Result := ct_options(con, CS_GET, option, {$IFDEF XSQL_CLR}Value{$ELSE}PCS_VOID(@Value){$ENDIF}, CS_UNUSED, nil);
end;

function SybConSetOptionInt32(con: PCS_CONNECTION; option, Value: TCS_INT): TCS_RETCODE;
begin
  Result := ct_options(con, CS_SET, option, {$IFDEF XSQL_CLR}Value{$ELSE}PCS_VOID(@Value){$ENDIF}, CS_UNUSED, nil);
end;
  ...
  while ct_results( ACmd, rslt ) = CS_SUCCEED do begin
    case rslt of
      ...
      CS_CMD_DONE:      // the result of a logical command have been completely processed
        begin
          Check( ct_res_info( CmdPtr, CS_ROW_COUNT, {$IFNDEF XSQL_CLR}@{$ENDIF}FRowsAffected, CS_UNUSED, outlen ) );
          Check( CS_FAIL );
        end;
...
begin
  ...
  Check( ct_res_info( CmdPtr, CS_NUMDATA, {$IFNDEF XSQL_CLR}@{$ENDIF}NumCols, CS_UNUSED, outlen) );

  for Col:=1 to NumCols do begin
    InitDataFmtStruct(colinfo);
    Check( ct_describe( CmdPtr, Col, colinfo ) );
    if colinfo.namelen > 0 then
      sColName := {$IFDEF XSQL_CLR}colinfo.name{$ELSE}StrPas(colinfo.name){$ENDIF};
...
begin
  ...
  Check( cs_convert( pCSContext, datafmt, data, destfmt, {$IFNDEF XSQL_CLR}@{$ENDIF}Result, outlen ) );
end;
...
var
  DecSep: {$IFDEF XSQL_CLR}string{$ELSE}Char{$ENDIF};
begin
