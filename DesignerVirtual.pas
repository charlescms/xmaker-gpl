unit DesignerVirtual;

interface

uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs,
  ComCtrls, ExtCtrls, ToolWin, StdCtrls, Buttons, Menus, TypInfo, PopupLb;

type
  TFormDesignerVirtual = class;
  ...
  TFormDesignerVirtual = class(TForm)
    PnFundo: TPanel;
    ...
    procedure Pn_Barra_S_00Resize(Sender: TObject);
    procedure Pn_Barra_I_00Resize(Sender: TObject);
    ...
    procedure PaintBox1MouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure PageControl_ObjInspEnter(Sender: TObject);
    procedure PageControl_ObjInspExit(Sender: TObject);
    procedure PageControl_ObjInspResize(Sender: TObject);
...
var
  FormDesignerVirtual: TFormDesignerVirtual;
...
uses PicEdit, StrEdit, Rotinas, DEntrada, MiniEditor, ChildFrm, ChildFrm_DEntrada, FormVirtual,
  Princ;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                Conteudo := FormDesignerVirtual.GetPropAsString(TObject(GetOrdProp(Sender, Info)),PropList[I],True);
                inc(P);
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          Conteudo := FormDesignerVirtual.GetPropAsString(Comp,PropInfo,False);
          if UpperCase(Trim(PropList^[I].Name)) = 'VISIBLE' then
            if FrmVirtual.ListaInvisivel.IndexOf(UpperCase(Comp.Name)) > -1 then
              Conteudo := 'False';
          if UpperCase(Trim(PropList^[I].Name)) = 'ENABLED' then
            if FrmVirtual.ListaDesabilitado.IndexOf(UpperCase(Comp.Name)) > -1 then
              Conteudo := 'False';
          if Copy(Conteudo,01,01) <> '+' then
  ...
  try
    ...
    try
      ...
      begin
        ...
        Y := FrmVirtual.ListaEventos.IndexOf(Comp.Name + ':' + StrEvento);
        if Y < 0 then
          StrEvento := '';
        if I < List.Count then
...
begin
  ...
  if FrmVirtual.ListaSelecionados.Count = 1 then
...
{----------------------------------------------------------------------------}
procedure TFormDesignerVirtual.CreateParams(var Params: TCreateParams);
begin
  inherited;
  //Params.WndParent := frDesigner.Handle;
end;

function TFormDesignerVirtual.GetPropValue(Index: Integer): Variant;
begin
  Result := TProp(FItems.Objects[Index]).Value;
end;

procedure TFormDesignerVirtual.SetPropValue(Index: Integer; Value: Variant);
begin
  TProp(FItems.Objects[Index]).Value := Value;
end;

procedure TFormDesignerVirtual.ClearProperties(Tudo: Boolean);
var
  i: Integer;
begin
...
procedure TFormDesignerVirtual.Inicializa(Sender: TObject);
begin
  PropInspector.Update(Sender);
end;

procedure TFormDesignerVirtual.AddProperty(PropName: String; PropValue: Variant;
  PropEnum: TStringList; PropEnumValues: Variant;
  PropEditor: TNotifyEvent);
begin
...
procedure TFormDesignerVirtual.InsertProperty(Index: Integer; PropName: String; PropValue: Variant;
  PropEnum: TStringList; PropEnumValues: Variant;
  PropEditor: TNotifyEvent);
begin
...
function TFormDesignerVirtual.CurItem: TProp;
begin
  Result := nil;
...
procedure TFormDesignerVirtual.SetItems(Value: TStringList);
begin
  FItems.Assign(Value);
  FItemIndex := -1;
  PaintBox1.Repaint;
...
procedure TFormDesignerVirtual.SetItemValue(Value: String);
var
  p: TProp;
  Tipo, Nome: String;
...
begin
  ...
  begin
    ...
    begin
      y := FrmVirtual.ListaInvisivel.IndexOf(UpperCase(PropInspector.VControl.Name));
      if UpperCase(Trim(Edit1.Text)) = 'TRUE' then
      begin
        if y > -1 then
          FrmVirtual.ListaInvisivel.Delete(y);
      end
      else
      begin
        if y < 0 then
          FrmVirtual.ListaInvisivel.Add(UpperCase(PropInspector.VControl.Name));
      end;
    end
    else if (UpperCase(Trim(Nome)) = 'ENABLED') and (UpperCase(Copy(FItems[FItemIndex],01,04)) = '   E') then
    begin
      y := FrmVirtual.ListaDesabilitado.IndexOf(UpperCase(PropInspector.VControl.Name));
      if UpperCase(Trim(Edit1.Text)) = 'TRUE' then
      begin
        if y > -1 then
          FrmVirtual.ListaDesabilitado.Delete(y);
      end
      else
      begin
        if y < 0 then
          FrmVirtual.ListaDesabilitado.Add(UpperCase(PropInspector.VControl.Name));
      end;
    end
    else
    begin
  ...
  //FrmVirtual.DsnRegister.DsnStage.UpdateControl;
end;

function TFormDesignerVirtual.GetItemValue(i: Integer): String;
var
  p: TProp;
begin
  Result := '';
...
procedure TFormDesignerVirtual.SetItemIndex(Value: Integer);
var
  ww, y: Integer;
  b1, b2: Boolean;
begin
...
function TFormDesignerVirtual.GetCount: Integer;
begin
  Result := FItems.Count;
end;

procedure TFormDesignerVirtual.ItemsChanged;
var
  LastIndex: Integer;
begin
  FItemIndex := -1;
...
procedure TFormDesignerVirtual.DrawOneLine(i: Integer; a: Boolean);
var
  R, R1: TRect;
...
procedure TFormDesignerVirtual.PaintBox1Paint(Sender: TObject);
var
  i: Integer;
  r: TRect;
begin
  ...
  try
    ...
    //FrmVirtual.Thread_ok := True;
  end;
end;

procedure TFormDesignerVirtual.FormShow(Sender: TObject);
begin
  Posicao_Unit := 0;
  if ClientHeight < 20 then
    CB1.Hide;
  case FrmVirtual.Tipo of
    1..2: TChildForm_DEntrada.CreateForm(TFormEntradaDados, PageControl_Forms, FrmVirtual.NomeForm, '');
    4: TChildForm.CreateForm(TFormulario_Virtual, PageControl_Forms, FrmVirtual.NomeForm, '');
  end;
end;

procedure TFormDesignerVirtual.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action := Cafree;
  PropInspector.Free;
  FormDesignerVirtual := Nil;
  FormPrincipal.BarraAssistente.Visible := FormPrincipal.BrAssistente.Checked;
end;

procedure TFormDesignerVirtual.FormCreate(Sender: TObject);
begin
  FormPrincipal.BarraAssistente.Visible := False;
...
procedure TFormDesignerVirtual.WMNCLButtonDblClk(var Message: TMessage);
begin
  if Height > 30 then
  begin
    ClientHeight := 0;
...
function TFormDesignerVirtual.GetClassName(ObjName: String): String;
begin
...
procedure TFormDesignerVirtual.Atribui(Sender: TObject; Tudo: Boolean);
var
  I,Y: Integer;
begin
  //if FormEntradaDados.ExcluiComp then
  //  FormEntradaDados.AtualizaLista_CB;
...
procedure TFormDesignerVirtual.FormResize(Sender: TObject);
begin
...
procedure TFormDesignerVirtual.Grow;
begin
  Show;
  if ClientHeight < 20 then
  begin
...
function TFormDesignerVirtual.GetPropAsString(Obj: TObject; Info: PPropInfo; SubItem: Boolean): string;
var
  Count,I: Integer;
  IntVal: LongInt;
  PropList: PPropList;
...
procedure TFormDesignerVirtual.FormDestroy(Sender: TObject);
begin
  b.Free;
  LB1.Free;
...
procedure TFormDesignerVirtual.CB1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = 13 then CB1Click(Self);
end;

procedure TFormDesignerVirtual.CB1Click(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    I := 0;
    while I <= FrmVirtual.Form.ComponentCount-1 do
    begin
      if FrmVirtual.Form.Components[I].Name = NomeComp then
      begin
        Componente := FrmVirtual.Form.Components[I];
        I := 9999;
      end;
      Inc(I);
    end;
    ...
    begin
      ...
      if not FormEntradaDados.VamosGravar then
      begin
        if Copy(Control.Parent.Name,01,08) = 'DsnStage' then
        begin
          NrPg := StrToIntDef(Trim(Copy(Control.Parent.Name,09,02)),0);
          if (NrPg > -1) and (NrPg < FormEntradaDados.TabPaginas.Tabs.Count) then
          begin
            FormEntradaDados.NoManutencao.PageIndex := NrPg;
            FormEntradaDados.TabPaginas.TabIndex    := NrPg;
          end;
        end;
      end;
      FormEntradaDados.ObjetoAtual := Control;
      if not FormEntradaDados.VamosGravar then
        FormEntradaDados.DsnSelect.Select(Control)
      else
...
procedure TFormDesignerVirtual.LB1Click(Sender: TObject);
begin
...
procedure TFormDesignerVirtual.Pn_Barra_S_00Resize(Sender: TObject);
begin
...
procedure TFormDesignerVirtual.Pn_Barra_I_00Resize(Sender: TObject);
begin
...
procedure TFormDesignerVirtual.Btn_S_FClick(Sender: TObject);
begin
  Pn_Barra_S_00.Visible := False;
  PageControl_Objetos.Visible := False;
  Splitter_D_J.Visible := False;

  if (not Pn_Barra_S_00.Visible) and (not Pn_Barra_I_00.Visible) then
  begin
...
procedure TFormDesignerVirtual.Btn_I_FClick(Sender: TObject);
begin
  Pn_Barra_I_00.Visible := False;
  PageControl_ObjInsp.Visible := False;
...
procedure TFormDesignerVirtual.PaintBox1DblClick(Sender: TObject);
var
  PropName:string;
  PropInfo:PPropInfo;
  I:Integer;
...
procedure TFormDesignerVirtual.PaintBox1MouseDown(Sender: TObject;
...
procedure TFormDesignerVirtual.PaintBox1MouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
  if not FDown then
...
procedure TFormDesignerVirtual.PaintBox1MouseUp(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  FDown := False;
end;

procedure TFormDesignerVirtual.PageControl_ObjInspEnter(Sender: TObject);
begin
  if Edit1.Enabled and Edit1.Visible then
    Edit1.SetFocus;
end;

procedure TFormDesignerVirtual.PageControl_ObjInspExit(Sender: TObject);
begin
  if BusyFlag then Exit;
  LB1.Hide;
...
procedure TFormDesignerVirtual.PageControl_ObjInspResize(Sender: TObject);
begin
  Box.Width := PageControl_ObjInsp.ClientWidth;
  Box.Height := PageControl_ObjInsp.ClientHeight - CB1.Height - 7; //- ControlBar2.Height;
  CB1.Width := PageControl_ObjInsp.ClientWidth;
...
procedure TFormDesignerVirtual.ComboBtnClick(Sender: TObject);
var
...
procedure TFormDesignerVirtual.EditBtnClick(Sender: TObject);
var
  Prop: TProp;
  ob: TObject;
...
begin
  ...
  begin
    ...
    Try
      Pesquisa := 'procedure TForm' + FormEntradaDados.NomeForm + '.' + StrEvento + ';';
      ...
      FormEntradaDados.TextoPAS.CaretX := 1;
      FormEntradaDados.TextoPAS.CaretY := 1;
      InicioEvento := 0;
      FormEntradaDados.TextoPAS.SearchReplace(Pesquisa, '', FormEntradaDados.SearchOptionsPd );
      if FormEntradaDados.TextoPAS.CaretY > 1 then
      begin
        Qtd_Begin := 0;
        Qtd_End := 0;
        Qtd_I_Coment := 0;
        Qtd_F_Coment := 0;
        with FormEntradaDados.TextoPAS do
        begin
          InicioEvento := CaretY;
          Fim := False;
          Inicio := False;
          ...
          begin
            ...
            if (Copy(Pesquisa,01,04) = 'END.') or
               (Copy(Pesquisa,01,Pos('.',Pesquisa)) = 'PROCEDURE TFORM' + UpperCase(FormEntradaDados.NomeForm) + '.' ) or
               (Copy(Pesquisa,01,Pos('.',Pesquisa)) = 'FUNCTION TFORM' + UpperCase(FormEntradaDados.NomeForm) + '.' ) then
              Fim := True;
          end;
        end;
      ...
      begin
        ...
        begin
          ...
          if FormEntradaDados.TextoPAS.CaretY > 1 then
          begin
            P := FormEntradaDados.TextoPAS.CaretY - 1;
            Fim := false;
            IntVal := 0;
            while not Fim do
            begin
              if Pos(');',RetiraBrancos(FormEntradaDados.TextoPas.Lines[P])) > 0 then Fim := True;
              FormEntradaDados.TextoPAS.Lines.Delete(P);
              Inc(IntVal);
            end;
          end;
          FormEntradaDados.TextoPAS.CaretY := InicioEvento;
          P := FormEntradaDados.TextoPAS.CaretY - IntVal ;
          for I:=0 to Qtd_Linhas_Original+InicioBloco-InicioEvento+1 do
            FormEntradaDados.TextoPas.Lines.Delete(P - 1);
          P := FormEntradaDados.ListaEventos.IndexOf(TControl(PropInspector.VControl).Name + ':' + Copy(StrEvento,01,Pos('(',StrEvento)-1));
          if P > -1 then
          begin
            FormEntradaDados.ListaEventos.Delete(P);
            Edit1.Text := '';
        ...
        begin
          FormEntradaDados.ListaEventos.Add(TControl(PropInspector.VControl).Name + ':' + Copy(StrEvento,01,Pos('(',StrEvento)-1));
          ...
          begin
            FormEntradaDados.TextoPAS.CaretY := InicioBloco;
            for I:=0 to Qtd_Linhas_Original do
              FormEntradaDados.TextoPas.Lines.Delete(FormEntradaDados.TextoPAS.CaretY);
            for I:=0 to FormMiniEditor.E_Metodo.Lines.Count-1 do
            begin
              FormEntradaDados.TextoPas.Lines.Insert(InicioBloco,FormMiniEditor.E_Metodo.Lines[I]);
              Inc(InicioBloco);
            end;
          end
          else
          begin
            Pesquisa := 'procedure TForm' + FormEntradaDados.NomeForm + '.' + StrEvento + ';';
            Pesquisa := Copy(Pesquisa,01,Pos('(',Pesquisa));
            FormEntradaDados.AbrirEditor(Projeto.Pasta + FormEntradaDados.NomeForm + '.PAS',Pesquisa,False,'',StrParametros+';',FormMiniEditor.E_Metodo.Lines);
          end;
        end;
        FormEntradaDados.TextoPAS.Modified := True;
      end;
    Finally
      FormMiniEditor.Free;
    end;
...
procedure TFormDesignerVirtual.Edit1Change(Sender: TObject);
var
  Tipo: String;
  ob: TObject;
  Nome: String;
...
procedure TFormDesignerVirtual.Edit1DblClick(Sender: TObject);
var
  p: TProp;
...
procedure TFormDesignerVirtual.Edit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if HideProperties then Exit;
...
procedure TFormDesignerVirtual.Edit1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
{$WARNINGS OFF}
procedure TFormDesignerVirtual.Edit1MouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
