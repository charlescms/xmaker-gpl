      ...
      try
        ...
        case Hor of
            ...
            begin
              ...
              for i:=0 to Pred(ControlCount) do
                if not IsLocked(Controls[i]) then
                  Controls[i].Left:=TControl(Controls[ControlCount-1]).Left; // TControl(Items[0]).Left
            end;
          amCenters:
            if ControlCount>1 then
            begin
              Sort(LeftSort);
              Val:=TControl(Controls[ControlCount-1]).Left;
              Sort(RightSort);
          ...
          amSpace:
            ...
            begin
              Sort(LeftSort);
              Val:=(TControl(Items[Pred(Count)]).Left-TControl(Controls[ControlCount-1]).Left) div Pred(Count);
              for i:=0 to Pred(Count) do
                TControl(Items[i]).Left:=TControl(Controls[ControlCount-1]).Left+Val*i;
            end;
          amWindowCenter:
          begin
        ...
        case Ver of
          amLeftTop:
            ...
            begin
              ...
              for i:=0 to Pred(ControlCount) do
                if not IsLocked(Controls[i]) then
                  Controls[i].Top:=TControl(Controls[ControlCount-1]).Top;
            end;
          amCenters:
            if ControlCount>1 then
            begin
              Sort(TopSort);
              Val:=TControl(Controls[ControlCount-1]).Top;
              Sort(BottomSort);
          ...
          amSpace:
            ...
            begin
              Sort(TopSort);
              Val:=(TControl(Items[Pred(Count)]).Top-TControl(Controls[ControlCount-1]).Top) div Pred(Count);
              for i:=0 to Pred(Count) do
                TControl(Items[i]).Top:=TControl(Controls[ControlCount-1]).Top+Val*i;
            end;
          amWindowCenter:
          begin
...
procedure TCustomFormDesigner.Select;
var
  i: Integer;
  xControl: TControl;
begin
        ...
        with ParentForm do
          for i:=0 to Pred(ComponentCount) do
            if (Components[i] is TControl) and
              ...
              (TControl(Components[i]).Parent = xControl) then
                AddControl(TControl(Components[i]));
      DeleteControl(xControl);
    end;
  finally
    SynchroUnlock;
  end;
