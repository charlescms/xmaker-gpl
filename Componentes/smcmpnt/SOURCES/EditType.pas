...
begin
  ...
  if FEnabledTypedLimit and not (Key in [#0, #8, #13]) then
  begin
    case FTypeValue of
      teNumber: begin
                  if (Key in ['.', ',']) then
                  ...
                  boolCallInherited := (((Key = DecimalSeparator) or (Key in SignOnly)) and (Pos(Key, Text) = 0))
                                       or (Key in Numeric) or (Key in ['E', 'e']);
                end;

      teInteger: boolCallInherited := ((Key in SignOnly) and (Pos(Key, Text) = 0))
                                  or (Key in Numeric);
      ...
      teShortDate,
      teTime,
      teShortTime: boolCallInherited := (Key in Numeric);
