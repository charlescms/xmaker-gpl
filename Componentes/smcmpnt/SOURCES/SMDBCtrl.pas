{ Copyright (C) 1998-2004, written by Shkolnik Mike, Scalabium
  ...
  Компонент TSMDBNavigator является расширением стандартного
  навигатора TDBNavigator и предоставляет дополнительные функции:
   - кроме стандартных кнопок (First/Nex/.../Refresh) добавлены
     новые кнопки Filter/Find/Print/Export, а также
     Choose для выбора записи и
     Clear для сброса выбранного значения записи
   - руссифицированы Hint'ы
   - кроме картинки есть возможность задавать наименования кнопкам в Captions
   - включение/выключение режима отображения наименований кнопок ShowCaption
   - возможность задавать расположение картинки в Layout
   - возможность выключения в BeforeAction обработки нажатия клавиши и не выполнения Click
     или изменения нажатой кнопки
}
...
uses
  Windows, Messages, Classes, Controls, ExtCtrls, DB, Buttons, SMCnst
...
type
  ...
  TSMNavigateBtn = (sbFirst, sbPrior, sbNext, sbLast,
                    ...
                    sbFilter, sbFind, sbPrint, sbExport,
...
var
  ...
  BtnCaption: array[TSMNavigateBtn] of PChar =
    ...
    SFilterName, SFindName, SPrintName, SExportName,
  ...
  BtnHintId: array[TSMNavigateBtn] of PChar = (SFirstRecord, SPriorRecord, SNextRecord, SLastRecord,
    ...
    SFilterRecord, SFindRecord, SPrintRecord, SExportRecord,
...
begin
  ...
  boolShift := True;
  if (sbChoice in VisibleButtons) or
  ...
  begin
    ...
    begin
      ...
      if ((i = sbChoice) or (i = sbClear)) and boolShift then
      begin
