...
type
  ...
  published
    ...
    property ColorFore: TColor index 0 read FColorFore write SetColor default clBlack;
    property ColorBack: TColor index 1 read FColorBack write SetColor default clWhite;
    property ColorRed: TColor index 2 read FColorRed write SetColor default clRed;
    property ColorYellow: TColor index 3 read FColorYellow write SetColor default clYellow;
...
procedure Register;
begin
  RegisterComponents('SMComponents', [TStopLightSensor, TAnalogSensor]);
end;
...
procedure TSensorPanel.SetColor(Index: Integer; AValue: TColor);
begin
  ...
  begin
    if (AValue > FValueMin) then
      if not (csLoading in ComponentState) then
        raise EInvalidOperation.CreateFmt(SOutOfRange, [-MaxInt, Round(FValueMax - 1)]);
    FValueMin := AValue;
...
begin
  ...
  begin
    ...
    case AValue of
      slUNKNOWN: Picture.Bitmap.Handle := LoadBitmap(hInstance, 'STOP_UNKNOWN');
      slRED: Picture.Bitmap.Handle := LoadBitmap(hInstance, 'STOP_RED');
      slYELLOW: Picture.Bitmap.Handle := LoadBitmap(hInstance, 'STOP_YELLOW');
      slGREEN: Picture.Bitmap.Handle := LoadBitmap(hInstance, 'STOP_GREEN');
    end;
  end;
end;
