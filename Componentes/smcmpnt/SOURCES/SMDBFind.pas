...
begin
  ...
  if FShowDialog then
  begin
    frmFind := TfrmFind.Create(Self.Owner{Application});
    with frmFind do
      try
...
//          cbMode.ItemIndex := 1;
  ...
  {if search is by one field from at beginning of a string or not any position, then LOCATE}
   ...
   if FSayResult then
     case Result of
//       frFound: MessageDlg('Record is found!', mtInformation, [mbOk], 0);
       frNotFound: MessageDlg('Record is not found by your criteria!', mtInformation, [mbOk], 0);
       frError: MessageDlg('Error is occured during search!', mtError, [mbOk], 0);
     end;
