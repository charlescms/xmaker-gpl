...
function ErrHandler(dbproc: PDBPROCESS; severity, dberr, oserr: INT; dberrstr, oserrstr: TSDCharPtr): INT; {$IFNDEF SD_CLR}cdecl;{$ENDIF}
var
  sStr: string;
begin
...
const
  SizeOfDBDATETIME = {$IFDEF SD_CLR} SizeOf(DBINT)+SizeOf(ULONG) {$ELSE} SizeOf(DBDATETIME) {$ENDIF};
...
begin
  Result :=
  	FieldType in [ftBoolean,
        	 ...
        	 ftCurrency, ftFloat {$IFDEF SD_VCL5} , ftGuid {$ENDIF}];
end;
...
begin
  ...
  for Col:=1 to NumCols do begin
    ...
    dbcol.SizeOfStruct := {$IFDEF SD_CLR}Marshal.{$ENDIF}SizeOf(dbcol);
...
{$ENDIF}
  ...
  szDBName := SDMss.dbname( SqlDatabase.DBProcPtr );
