  ...
  if Owner <> nil then
    for I := 0 to Owner.ComponentCount - 1 do begin
      ...
      if (Component is TSDDataBase) and
         (CompareText(TSDDatabase(Component).SessionName, Self.SessionName) <> 0)
      then
...
begin
  for i := 0 to FDatabases.Count - 1 do begin
    Result := TSDDatabase( FDatabases[i] );
...
begin
  Result := TSDDatabase( FDatabases[Index] );
end;
...
procedure TSDSession.GetFieldNames(const DatabaseName, TableName: string; List: TStrings);
var
  db: TSDDatabase;
begin
  db := FindDatabase(DatabaseName);
...
procedure TSDDatabase.RegisterClient(Client: TObject; Event: TConnectChangeEvent = nil);
begin
  inherited;
end;

procedure TSDDatabase.UnRegisterClient(Client: TObject);
begin
  inherited;
end;
...
function TSDDatabase.ConnectedStored: Boolean;
begin
  Result := Connected and (ddoStoreConnected in FDesignOptions);
end;

procedure TSDDatabase.SetDesignOptions(Value: TSDDesignDBOptions);
begin
...
function TSDDatabase.GetInTransaction: Boolean;
begin
  Result := False;
...
procedure TSDDatabase.GetFieldNames(const TableName: string; List: TStrings);
begin

  ISqlGetFieldNames(TableName, List);
end;

procedure TSDDatabase.GetTableNames(const Pattern: string; SystemTables: Boolean; List: TStrings);
begin
...
procedure TSDDatabase.InitSqlDatabase(const ADatabaseName, AUserName, APassword: string; AHandle: PSDCursor);
begin
...
procedure TSDDatabase.DoneSqlDatabase;
begin
...
function TSDDatabase.TestConnected: Boolean;
begin
  Result := Connected;
  if Result then
    try
...
function TSDDatabase.ISqlParamValue(Value: TSDDatabaseParam): Integer;
begin
  Result := FSqlDatabase.ParamValue(Value);
end;

procedure TSDDatabase.ISqlGetStoredProcNames(List: TStrings);
var
  cmd: TISqlCommand;
...
procedure TSDDatabase.ISqlGetTableNames(Pattern: string; SystemTables: Boolean; List: TStrings);
var
  cmd: TISqlCommand;
  OwnerNo, NameNo: Integer;
  sOwner, sName: string;
...
procedure TSDDatabase.ISqlGetFieldNames(const TableName: string; List: TStrings);
var
  cmd: TISqlCommand;
  NameNo: Integer;
  s: string;
...
function TSDDatabase.GetSchemaInfo(ASchemaType: TSDSchemaType; AObjectName: string): TDataSet;
var
  cmd: TISqlCommand;
  q: TSDQuery;
begin
...
procedure TSDDatabase.IdleTimeOutReset(Sender: TObject);
begin
  FIdleTimeOutStarted := False;
end;

procedure TSDDatabase.BusyStateReset(Sender: TObject);
begin
  SDEngine.ResetBusyState;
end;
          ...
          if DataSet.AutoRefresh and (UpdateKind = ukInsert) then begin
			...
			// only one field can be autoincremented
                for j:=0 to DataSet.FieldCount-1 do begin
                  if ((DataSet.Fields[j].DataType = ftAutoInc) {$IFDEF SD_VCL5} or (DataSet.Fields[j].AutoGenerateValue = arAutoInc) {$ENDIF})
...
begin
  if csReading in ComponentState then
    ...
    FDatabaseName := Value;
    DataEvent(dePropertyChange, {$IFDEF SD_CLR} nil {$ELSE} 0 {$ENDIF});
  end;
end;
...
const
  TempStateSet = [dsBrowse, dsEdit, dsInsert, dsCalcFields{$IFDEF SD_VCL4}, dsBlockRead{$ENDIF}];
var
  RecBuf: TSDRecordBuffer;
...
begin
  with Field do begin
    ...
    if not (State in [dsCalcFields, dsFilter, dsNewValue]) then
      DataEvent(deFieldChange, {$IFDEF SD_CLR} Field {$ELSE} Longint(Field) {$ENDIF});
  end;
end;
...
begin
  Result := SDCommon.BCDToCurr( TBcd(BCD^), Curr );
end;

function TSDDataSet.CurrToBCD(const Curr: Currency; BCD: TSDPtr; Precision, Decimals: Integer): Boolean;
begin
  Result := SDCommon.CurrToBCD( Curr, TBcd(BCD^), Precision, Decimals );
end;
...
procedure TSDDataSet.InternalGotoBookmark({$IFDEF SD_CLR} const {$ENDIF} Bookmark: TBookmark);
var
  RecPos: Integer;
begin
...
procedure TSDDataSet.GetBookmarkData(Buffer: TSDRecordBuffer; {$IFDEF SD_CLR} var Data: TBookmark {$ELSE} Data: TSDPtr {$ENDIF});
begin
  SafeCopyMem( TSDPtr(Longint(Buffer) + FBookmarkOffs), Data, BookmarkSize );
end;

procedure TSDDataSet.SetBookmarkData(Buffer: TSDRecordBuffer; {$IFDEF SD_CLR} const Data: TBookmark {$ELSE} Data: TSDPtr {$ENDIF});
begin
  SafeCopyMem( Data, TSDPtr(Longint(Buffer) + FBookmarkOffs), BookmarkSize );
end;

function TSDDataSet.BookmarkValid({$IFDEF SD_CLR} const {$ENDIF} Bookmark: TBookmark): Boolean;
var
  InCacheNo: Integer;
begin
  Result := False;
...
function TSDDataSet.CompareBookmarks({$IFDEF SD_CLR} const {$ENDIF} Bookmark1, Bookmark2: TBookmark): Integer;
const
  BkmLess           = -1;             { Bkm1 < Bkm2 }
...
begin
  ...
  DataEvent(deDataSetScroll, {$IFDEF SD_CLR} TObject(Integer(-1)) {$ELSE} -1 {$ENDIF});
end;
{$ENDIF}

{ Datasource/Datalink Interaction }

procedure TSDDataSet.DataEvent(Event: TDataEvent; Info: {$IFDEF SD_CLR} TObject {$ELSE} Longint {$ENDIF});
begin
...
procedure TSDDataSet.DoSortRecords(AFields: array of Integer; AscOrder, CaseSensitive: array of Boolean);
  ...
  begin
    ...
    if FldCount = 1 then
      ...
      Result := VarArrayCreate([0, FldCount-1], {$IFDEF SD_CLR}varObject{$ELSE}varVariant{$ENDIF});
...
begin
  ...
  if (OpMode = usUnmodified) and (Trim(ARefreshSQL) <> '') then
    ...
    sStmt := (UpdateObject as TSDUpdateSQL).SQL[OpMode].Text
  else begin
  ...
  try
    ...
    sql.Params := TSDHelperParams.Create {$IFDEF SD_VCL4} (Self) {$ENDIF};
      ...
      if AutoRefresh and (OpMode = usInserted) then begin
		...
		// only one field can be autoincremented
              for i:=0 to FieldCount-1 do begin
                if ((Fields[i].DataType = ftAutoInc) {$IFDEF SD_VCL5}or (Fields[i].AutoGenerateValue = arAutoInc) {$ENDIF})
                   and Fields[i].IsNull
                then begin
...
begin
  ...
  if IsBlobType( AFieldDesc.FieldType ) then begin
    ...
    bCnvtToInt := Assigned(f) and (f.DataType <> AFieldDesc.FieldType) and
                  ...
                  (AFieldDesc.FieldType in [ftSmallint, ftWord, ftAutoInc, ftFloat{$IFDEF SD_VCL4}, ftLargeInt{$ENDIF}]);
      ...
      if bCnvtDateTime then begin
        ...
        if f.DataType = ftDate then begin
          if AFieldDesc.FieldType = ftDate then
            ...
            dtValue := MSecsToTimeStamp( {$IFDEF SD_CLR} Trunc(dtDateTimeRec.DateTime) {$ELSE} dtDateTimeRec.DateTime {$ENDIF} ).Date;
        end else if f.DataType = ftTime then begin
          if AFieldDesc.FieldType = ftTime then
            dtValue := dtDateTimeRec.Time
          else
            dtValue := MSecsToTimeStamp( {$IFDEF SD_CLR} Trunc(dtDateTimeRec.DateTime) {$ELSE} dtDateTimeRec.DateTime {$ENDIF} ).Time;
        end else
...
function TSDDataSet.PSExecuteStatement(const ASQL: string; AParams: TParams; {$IFDEF SD_CLR} var ResultSet: TObject {$ELSE} ResultSet: TSDPtr = nil {$ENDIF}): Integer;
var
  InProvider: Boolean;
  qryStat: TSDQuery;
begin
...
begin
  ...
  end else if Assigned(UpdateObject) and (UpdateObject is TSDUpdateSQL) then begin
{$IFDEF SD_CLR}
    TSDUpdateSQL(FUpdateObject).Dataset := Delta as TSDDataSet;
    TSDUpdateSQL(FUpdateObject).Apply(UpdateKind);
    Result := True;
{$ELSE}
    SQL := TSDUpdateSQL(FUpdateObject).GetSQL(UpdateKindToStatus(UpdateKind)).Text;
    if SQL <> '' then begin
...
procedure TSDQuery.ReleaseHandle(SaveRes: Boolean);
begin
...
procedure TSDQuery.InternalPost;
begin
  inherited;
...
procedure TSDQuery.InternalDelete;
begin
  if RequestLive then
...
procedure TSDQuery.SetUpdateMode(const Value: TUpdateMode);
begin
  CheckInactive;
...
function TSDQuery.IsExecDirect: Boolean;
begin
...
procedure TSDQuery.CreateHandle;
var
  i, j: Integer;
  fl, tl: TStringList;
...
procedure TSDQuery.SetPrepareCmd(Value, GenCursor: Boolean);
begin
...
begin
  ...
  if not (csReading in ComponentState) then begin
    ...
    DataEvent(dePropertyChange, {$IFDEF SD_CLR} nil {$ELSE} 0 {$ENDIF});	// FieldDefs.Updated := False - not valid
  end else
    CreateParamsFromSQL(nil, Text, DefaultParamPrefix);
end;

{$IFDEF SD_VCL4}
procedure TSDQuery.GetDetailLinkFields(MasterFields, DetailFields: TObjectList);
...
-------------------------------------------------------------------------------}
procedure TSDQuery.SetSqlCmd(Value: TISqlCommand);
begin
  Close;
...
function TSDQuery.ISqlCmdCreate: TISqlCommand;
begin
...
function TSDQuery.GetIsIndexField(Field: TField): Boolean;
begin
...
procedure TSDQuery.UpdateUniqueIndexInfo;
var
  ds: TDataSet;
...
{*******************************************************************************
 		TSDMacroQuery
*******************************************************************************}
constructor TSDMacroQuery.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  ...
  FMacros := TSDHelperParams.Create {$IFDEF SD_VCL4} (Self) {$ENDIF};
end;

destructor TSDMacroQuery.Destroy;
begin
  inherited;
...
procedure TSDMacroQuery.DefineProperties(Filer: TFiler);

  function HasMacroData: Boolean;
  begin
    if Filer.Ancestor <> nil
    then Result := not FMacros.IsEqual(TSDMacroQuery(Filer.Ancestor).FMacros)
...
procedure TSDMacroQuery.ReadMacroData(Reader: TReader);
begin
  Reader.ReadValue;
  Reader.ReadCollection(FMacros);
end;

procedure TSDMacroQuery.WriteMacroData(Writer: TWriter);
begin
...
function TSDMacroQuery.GetMacroCount: Word;
begin
  Result := FMacros.Count;
end;

function TSDMacroQuery.GetMacros: TSDHelperParams;
begin
  Result := FMacros;
end;

function TSDMacroQuery.MacroByName(const MacroName: string): TSDHelperParam;
begin
  Result := FMacros.ParamByName(MacroName);
end;

procedure TSDMacroQuery.SetMacroChar(const Value: Char);
begin
...
procedure TSDMacroQuery.SetMacros(const Value: TSDHelperParams);
begin
  FMacros.AssignValues(Value);
end;

{ Sets an unexpanded(FSQLPattern) and a real(expanded)(inherited SQL) statements }
procedure TSDMacroQuery.SetQuery(Value: TStrings);
begin
...
procedure TSDMacroQuery.QueryChanged(Sender: TObject);
begin
...
procedure TSDMacroQuery.PatternChanged(Sender: TObject);
begin
  CreateMacros;
...
procedure TSDMacroQuery.CreateMacros;
var
  List: TSDHelperParams;
  i: Integer;
begin
...
procedure TSDMacroQuery.ExpandMacros;
var
  i: Integer;
  s: string;
begin
...
function TSDMacroQuery.SetDSFlag(Flag: Integer; Value: Boolean): Boolean;
begin
...
function TSDUpdateSQL.GetQueryEx(StmtKind: TUpdateStatus): TSDQuery;
begin
  if not Assigned(FQueries[StmtKind]) then begin
    FQueries[StmtKind] := TSDQuery.Create(Self);
...
function TSDUpdateSQL.GetSQL(StmtKind: TUpdateStatus): TStrings;
begin
...
procedure TSDUpdateSQL.SetSQL(StmtKind: TUpdateStatus; Value: TStrings);
begin
...
begin
  ...
  if FModified then
  try
    FDataSet.DataEvent(deFieldChange, {$IFDEF SD_CLR} FField {$ELSE} Longint(FField) {$ENDIF});
  except
...
function TSDBlobStream.Read(var Buffer {$IFDEF SD_CLR}: TBytes; Offset,{$ELSE}; {$ENDIF} Count: LongInt): Longint;
var
  sBlob: TSDBlobData;
begin
  Result := 0;
...
function TSDBlobStream.Write(const Buffer {$IFDEF SD_CLR}: TBytes; Offset,{$ELSE}; {$ENDIF} Count: Longint): Longint;
var
  sBlob: TSDBlobData;
...
procedure TSDBlobStream.SetSize(NewSize: {$IFDEF SD_CLR}Int64 {$ELSE} Longint {$ENDIF});
var
  sBlob: TSDBlobData;
begin
...
procedure TSDStoredProc.CreateHandle;
begin
  FSqlCmd := ISqlCmdCreate;
...
procedure TSDStoredProc.ExecuteCursor;
begin
        // it is not necessary to execute a statement, when IsExecDirect is True
        // and the statement was executed in CreateHandle, in this case FExecCmd = False.
        // FExecCmd - is it necessary to execute statement here.
...
procedure TSDStoredProc.CloseCursor;
begin
  inherited;
...
{ It's required to release DB handle for some connections(for example, Mss, Sybase, ODBC)}
procedure TSDStoredProc.ReleaseHandle(SaveRes: Boolean);
begin
  if Active and SaveRes then
    FetchAll;
end;
...
function TSDStoredProc.IsExecDirect: Boolean;
begin
  Result := ((dsfOpened in DSFlags) or (dsfExecProc in DSFlags)) and not(dsfStoredProc in DSFlags);
end;

procedure TSDStoredProc.SetPrepareCmd(Value, GenCursor: Boolean);
begin
...
{-------------------------------------------------------------------------------
			Common SQL server C/API Functions
-------------------------------------------------------------------------------}
function TSDStoredProc.ISqlCmdCreate: TISqlCommand;
begin
  Result := inherited ISqlCmdCreate;
...
function TSDStoredProc.ISqlDescriptionsAvailable: Boolean;
var
  db: TSDDatabase;
begin
  ...
  if not db.Connected then
    db.Open;
  ASSERT( db.FSqlDatabase<>nil, 'TSDStoredProc.ISqlDescriptionsAvailable: FSqlDatabase=nil' );
  Result := db.FSqlDatabase.SPDescriptionsAvailable;
end;

function TSDStoredProc.ISqlNextResultSet: Boolean;
begin
  Result := False;
  if Assigned(SqlCmd) then
...
procedure TSDStoredProc.ISqlPrepareProc;
begin
  ISqlPrepare( StoredProcName );
end;

procedure TSDStoredProc.ISqlExecProcDirect;
begin
...
procedure TSDStoredProc.ISqlExecProc;
begin
  ISqlExecute;
	// MSSQL, Sybase & ODBC require to call FetchAll before this
  if not SqlCmd.SqlDatabase.ProcSupportsCursors then
...
procedure TSDStoredProc.ISqlGetResults;
begin
  if Assigned(SqlCmd) then
    SqlCmd.GetOutputParams;
end;
...
{ TSDTable }
{$IFDEF SD_VCL5}
constructor TSDTable.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
{ It's required to release DB handle for some connections(for example, Mss, Sybase, ODBC)}
procedure TSDTable.ReleaseHandle(SaveRes: Boolean);
begin
  if Active and SaveRes then
    FetchAll;
end;

function TSDTable.FieldDefsStored: Boolean;
begin
  Result := StoreDefs and (FieldDefs.Count > 0);
end;

function TSDTable.IndexDefsStored: Boolean;
begin
  Result := StoreDefs and (IndexDefs.Count > 0);
end;

function TSDTable.GetIndexFieldNames: string;
begin
  if FFieldsIndex then Result := FIndexName else Result := '';
end;

function TSDTable.GetIndexName: string;
begin
  if FFieldsIndex then Result := '' else Result := FIndexName;
end;

procedure TSDTable.GetIndexNames(List: TStrings);
begin
  IndexDefs.Update;
  IndexDefs.GetItemNames(List);
end;

procedure TSDTable.SetIndexDefs(Value: TIndexDefs);
begin
  IndexDefs.Assign(Value);
end;

function TSDTable.GetIsIndexField(Field: TField): Boolean;
var
  i: Integer;
begin
  Result := False;
...
function TSDTable.GetIndexField(Index: Integer): TField;
var
  FieldNo: Integer;
begin
...
procedure TSDTable.SetIndexField(Index: Integer; Value: TField);
begin
  GetIndexField(Index).Assign(Value);
end;

function TSDTable.GetIndexFieldCount: Integer;
begin
  Result := FIndexFieldCount;
end;

procedure TSDTable.SetIndex(const Value: string; FieldsIndex: Boolean);
begin
...
procedure TSDTable.SetIndexFieldNames(const Value: string);
begin
  SetIndex(Value, Value <> '');
end;

procedure TSDTable.SetIndexName(const Value: string);
begin
  SetIndex(Value, False);
end;

procedure TSDTable.SetReadOnly(Value: Boolean);
begin
  CheckInactive;
  FReadOnly := Value;
end;

procedure TSDTable.SetTableName(const Value: string);
begin
  if csReading in ComponentState then
    FTableName := Value
...
procedure TSDTable.UpdateIndexDefs;
const
  DeltaFieldMap = 5;
...
procedure TSDTable.ClearIndexDescs;
begin
  SetLength(FIndexDescs, 0);
end;

procedure TSDTable.UpdateIndexDescs;
...
function TSDTable.GetCurrentIndex: Integer;
var
  i: Integer;
begin
  Result := -1;
...
function TSDTable.GetFieldListWithDesc(const AFields, ADescFields: string): string;
var
  i, j, s: Integer;
  Fld: string;
begin
...
function TSDTable.ReplaceSemicolon2Comma(const AStr: string): string;
var
  i: Integer;
begin
  Result := AStr;
...
function TSDTable.GenOrderBy: string;
var
...
procedure TSDTable.SwitchToIndex;
begin
  FOrderByFields := GenOrderBy;
end;

{ Different supported datatypes (map) and syntaxes(create/alter) could be implemented
through ini-file, TSDDatabase.Params or TISqlDatabase.SqlDataTypeName method.
...
procedure TSDTable.CreateTable;
const
  DT_ParamMarker = '?';
...
function TSDTable.GetExists: Boolean;
var
  List: TStrings;
begin
  List := nil;
...
procedure TSDTable.DeleteTable;
begin
  CheckInactive;
  SetDSFlag(dsfTable, True);
  try
...
procedure TSDTable.EmptyTable;
begin
  if Active then begin
    CheckBrowseMode;
    ISqlEmptyTable;
    ClearBuffers;
    DataEvent(deDataSetChange, {$IFDEF SD_CLR} nil {$ELSE} 0 {$ENDIF});
  end else begin
    SetDSFlag(dsfTable, True);
    try
      ISqlEmptyTable;
...
function TSDTable.GetCanModify: Boolean;
begin
  Result := not ReadOnly;
end;

function TSDTable.GetSQLText: string;
var
  s: string;
begin
  s := '';
...
procedure TSDTable.CreateHandle;
begin
  if FTableName = '' then DatabaseError(SNoTableName{$IFDEF SD_VCL4}, Self{$ENDIF});
...
procedure TSDTable.ExecuteCursor;
begin
  SetConnectionState( True );
  try
    ISqlExecute;
...
procedure TSDTable.InternalOpen;
begin
...
procedure TSDTable.InternalClose;
begin
  inherited;
...
procedure TSDTable.InternalDelete;
var
...
procedure TSDTable.InternalPost;
var
  CurrentIndex: Integer;
  SaveUpdateMode: TUpdateMode;
begin
...
procedure TSDTable.DataEvent(Event: TDataEvent; Info: {$IFDEF SD_CLR} TObject {$ELSE} Longint {$ENDIF});
begin
  inherited DataEvent(Event, Info);
...
procedure TSDTable.CheckMasterRange;
begin
...
procedure TSDTable.MasterChanged(Sender: TObject);
begin
  CheckBrowseMode;
  UpdateRange;
  ApplyRange;
...
procedure TSDTable.MasterDisabled(Sender: TObject);
begin
  CancelRange;
end;

function TSDTable.GetDataSource: TDataSource;
begin
  Result := FMasterLink.DataSource;
end;

procedure TSDTable.SetDataSource(Value: TDataSource);
begin
  if IsLinkedTo(Value) then DatabaseError(SCircularDataLink, Self);
  FMasterLink.DataSource := Value;
end;

function TSDTable.GetMasterFields: string;
begin
  Result := FMasterLink.FieldNames;
end;

procedure TSDTable.SetMasterFields(const Value: string);
begin
  FMasterLink.FieldNames := Value;
end;

procedure TSDTable.DoOnNewRecord;
var
  i: Integer;
begin
...
procedure TSDTable.ApplyRange;
begin
  CheckBrowseMode;
//  if SetCursorRange then First;
end;

procedure TSDTable.CancelRange;
begin
  CheckBrowseMode;
  UpdateCursorPos;
...
procedure TSDTable.UpdateRange;
begin
  SetLinkRanges(FMasterLink.Fields);
end;

procedure TSDTable.SetLinkRanges(MasterFields: TList);
var
  Field: TField;
  Param: TSDHelperParam;
  i, iFld: Integer;
...
begin
  ...
  while i <= Length(sDetailFields) do begin
    ...
    if iFld >= MasterFields.Count then
      DatabaseErrorFmt(SFatalError, ['TSDTable.SetLinkRanges: many fields in MasterFields property.']);
...
procedure TSDTable.RefreshParams;
var
  DataSet: TDataSet;
  i: Integer;
  IsValueEqual: Boolean; // whether parameter and the corresponded field values are equal
...
procedure TSDTable.SetParamsFromMasterDataSet;
var
  i: Integer;
begin
...
{-------------------------------------------------------------------------------
			SQL server C/API Functions
-------------------------------------------------------------------------------}
function TSDTable.ISqlCmdCreate: TISqlCommand;
begin
  Result := inherited ISqlCmdCreate;
  Result.CommandType := ctQuery;
  Result.Params := FParams;
...
procedure TSDTable.ISqlDeleteTable;
var
  cmd: TISqlCommand;
begin
  cmd := inherited ISqlCmdCreate;
...
procedure TSDTable.ISqlEmptyTable;
var
  cmd: TISqlCommand;
begin
  cmd := inherited ISqlCmdCreate;
...
{$ENDIF}     // implementation of TSDTable

{ TSDScript }

constructor TSDScript.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FSQL := TStringList.Create;
  FSQLPattern := TStringList.Create;
...
function TSDScript.GetDatabase: TSDDatabase;
begin
  Result := FQuery.Database;
end;

function TSDScript.GetDatabaseName: string;
begin
  Result := FQuery.DatabaseName;
end;

procedure TSDScript.SetDatabaseName(const Value: string);
begin
  FQuery.DatabaseName := Value;
end;

function TSDScript.GetSessionName: string;
begin
  Result := FQuery.SessionName;
end;

procedure TSDScript.SetSessionName(const Value: string);
begin
  FQuery.SessionName := Value;
end;

function TSDScript.GetDBSession: TSDSession;
begin
  Result := FQuery.DBSession;
end;

function TSDScript.GetMacroCount: Word;
begin
  Result := FMacros.Count;
end;

function TSDScript.GetMacros: TSDHelperParams;
begin
  Result := FMacros;
end;

function TSDScript.MacroByName(const MacroName: string): TSDHelperParam;
begin
  Result := FMacros.ParamByName(MacroName);
end;

procedure TSDScript.SetMacroChar(const Value: Char);
begin
  if Value <> FMacroChar then begin
    FMacroChar := Value;
    CreateMacros;
...
procedure TSDScript.SetMacros(const Value: TSDHelperParams);
begin
  FMacros.AssignValues(Value);
end;

function TSDScript.GetText: string;
begin
  Result := FSQL.Text;
end;

function TSDScript.GetParamsCount: Integer;
begin
  Result := FParams.Count;
end;

procedure TSDScript.SetParamsList(const Value: TSDHelperParams);
begin
  FParams.AssignValues(Value);
end;

procedure TSDScript.SetQuery(const Value: TStrings);
begin
...
procedure TSDScript.PatternChanged(Sender: TObject);
var
  List: TSDHelperParams;
begin
  if not (csReading in ComponentState) then begin
...
function TSDScript.ParamByName(const Value: string): TSDHelperParam;
begin
  Result := FParams.ParamByName(Value);
end;

{ execute all statements }
procedure TSDScript.ExecSQL;
begin
  ExpandMacros;
...
procedure TSDScript.ExecuteScript(StatementNo: Integer);
var
...
procedure TSDScript.CheckExecQuery(LineNo, StatementNo: Integer);
var
  i: Integer;
  Param: TSDHelperParam;
{$IFNDEF SD_CLR}
...
{ parses a statement for macros and fills FMacros list }
procedure TSDScript.CreateMacros;
var
  List: TSDHelperParams;
  i: Integer;
begin
...
{ Expands macros and assign a real SQL text }
procedure TSDScript.ExpandMacros;
var
  i: Integer;
  s: string;
begin
...
{$IFDEF SD_VCL4}
procedure TSDScript.DefineProperties(Filer: TFiler);

  function HasMacroData: Boolean;
  begin
    if Filer.Ancestor <> nil
    then Result := not FMacros.IsEqual(TSDScript(Filer.Ancestor).FMacros)
    else Result := FMacros.Count > 0;
  end;
  function HasParamData: Boolean;
  begin
    if Filer.Ancestor <> nil
    then Result := not FParams.IsEqual(TSDScript(Filer.Ancestor).FParams)
    else Result := FParams.Count > 0;
  end;
...
procedure TSDScript.ReadMacroData(Reader: TReader);
begin
  Reader.ReadValue;
  Reader.ReadCollection(FMacros);
end;

procedure TSDScript.WriteMacroData(Writer: TWriter);
begin
  Writer.WriteCollection(Macros);
end;

procedure TSDScript.ReadParamData(Reader: TReader);
begin
  Reader.ReadValue;
  Reader.ReadCollection(FParams);
end;

procedure TSDScript.WriteParamData(Writer: TWriter);
begin
  Writer.WriteCollection(Params);
end;
{$ENDIF}
...
{$IFDEF stSQLBase}      InitSqlDatabaseProcs[stSQLBase]	   := SDCsb.InitSqlDatabase;{$ENDIF}
{$IFDEF stOracle}       InitSqlDatabaseProcs[stOracle]	   := SDOra.InitSqlDatabase;{$ENDIF}
{$IFDEF stSQLServer}    InitSqlDatabaseProcs[stSQLServer]  := SDMss.InitSqlDatabase;{$ENDIF}
{$IFDEF stSybase}       InitSqlDatabaseProcs[stSybase] 	   := SDSyb.InitSqlDatabase;{$ENDIF}
{$IFDEF stDB2}          InitSqlDatabaseProcs[stDB2]	   := SDDb2.InitSqlDatabase;{$ENDIF}
{$IFDEF stInformix}     InitSqlDatabaseProcs[stInformix]   := SDInf.InitSqlDatabase;{$ENDIF}
{$IFDEF stODBC}         InitSqlDatabaseProcs[stODBC]	   := SDOdbc.InitSqlDatabase;{$ENDIF}
{$IFDEF stInterbase}    InitSqlDatabaseProcs[stInterbase]  := SDInt.InitSqlDatabase;{$ENDIF}
{$IFDEF stFirebird}     InitSqlDatabaseProcs[stFirebird]   := SDFib.InitSqlDatabase;{$ENDIF}
{$IFDEF stMySQL}        InitSqlDatabaseProcs[stMySql]      := SDMySql.InitSqlDatabase;{$ENDIF}
{$IFDEF stPostgreSQL}   InitSqlDatabaseProcs[stPostgreSQL] := SDPgSql.InitSqlDatabase;{$ENDIF}
{$IFDEF stOLEDB}        InitSqlDatabaseProcs[stOLEDB]      := SDOleDb.InitSqlDatabase;{$ENDIF}
