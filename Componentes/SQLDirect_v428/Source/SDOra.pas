...
type
  TOCICallbackInBind =
  	function(ictxp: Pdvoid; phBind: POCIBind; iter, index: ub4;
  		bufpp: PPdvoid; alenp: ub4p; piecep: ub1p; indpp: PPdvoid): sb4; {$IFNDEF SD_CLR}cdecl;{$ENDIF}

  TOCICallbackOutBind =
  	function(octxp: Pdvoid; phBind: POCIBind; iter, index: ub4;
        	bufpp: PPdvoid; alenpp: ub4pp; piecep: ub1p;
                indpp: PPdvoid; rcodepp: ub2pp): sb4; {$IFNDEF SD_CLR}cdecl;{$ENDIF}
...
{$IFNDEF SD_VCL5}             // TStringField in Delphi 4 does not support size 0 ('Invalid field size' error hapens)
...
{	implement, when it will be necessary
  ...
  peo := 0;
  if (AStmt <> nil) and
     (OCIAttrGet( Pdvoid(AStmt), OCI_HTYPE_STMT, {$IFNDEF SD_CLR}@{$ENDIF}peo, nil, OCI_ATTR_PARSE_ERROR_OFFSET, AErrHandle ) = OCI_SUCCESS)
  then
    ErrOffset := peo;
...
begin
  Result := {$IFDEF SD_CLR} False {$ELSE} @oexec <> nil {$ENDIF};
end;

function IsLoadedOCI8: Boolean;
begin
  Result := {$IFDEF SD_CLR} False {$ELSE} @OCIStmtExecute <> nil {$ENDIF};
end;
...
begin
  ...
  if index = 0 then begin
    Ora8Check( OCIAttrGet( Pdvoid(phBind), OCI_HTYPE_BIND, {$IFNDEF SD_CLR}@{$ENDIF}RetRows, nil,
...
{$ENDIF}
    try
	// logon
      Check( SDOra.olog(  rec.LdaPtr, rec.HdaPtr, szConnect, -1, nil, -1, nil, -1, OCI_LM_DEF  ) );
    finally
{$IFDEF SD_CLR}
...
{$ENDIF}
	...
	// open cursor
  Check( SDOra.oopen( rec.CdaPtr, PCdaDef(rec.LdaPtr), nil, -1, -1, nil, 0  ) );
...
begin
  ...
  rcd := SDOra.oclose( CurHandle );
  if not Force then
    Check( rcd );

  if InTransaction then
    SDOra.orol( PCdaDef( LdaPtr ) );
	// commit is automatically issued on a successful ologof call
  rcd := SDOra.ologof( LdaPtr );
  if not Force then
    Check( rcd );
...
begin
  Check( SDOra.ocom( PCdaDef( LdaPtr ) ) );
...
begin
  Check( SDOra.orol( PCdaDef( LdaPtr ) ) );
...
{$ENDIF}
  try
    Check( SDOra.oparse(CurHandle, szCmd, -1, DEFERRED_PARSE, ORACLE7_PARSE) );
    Check( SDOra.oexec(CurHandle) );
  finally
{$IFDEF SD_CLR}
...
begin
  ...
  try
    ...
    Check( SDOra.oopen( PCdaDef(FHandle), PCdaDef( SqlDatabase.LdaPtr ),
    				nil, -1, -1, nil, 0  ) );
  except
    FHandle := SafeReallocMem( FHandle, 0 );
    raise;
...
var
  rcd: sword;
begin
  rcd := SDOra.ofetch( PCdaDef(Handle) );
...
{$ENDIF}
      Check( SDOra.ogetpi(PCdaDef(Handle), Piece, ctxp, iter, index) );
	// read from database
      Len := GrowDelta;
      Check( SDOra.osetpi(PCdaDef(Handle), Piece, Buf, Len) );
      rcd := SDOra.ofetch(PCdaDef(Handle));
{$IFDEF SD_CLR}
...
{$ENDIF}
	// read from database
      ...
      Check( SDOra.oflng(PCdaDef(Handle), AFieldDesc.FieldNo, Buf, Len, dbtype, Len, BlobSize-GrowDelta) );
{$IFDEF SD_CLR}
      Marshal.Copy( Buf, BlobData, nOffset, Len );
{$ENDIF}
      Inc(Result, Len);
...
begin
  ...
  while True do begin
    ...
    if Count = 0 then
      Check( SDOra.osetpi(PCdaDef(Handle), Piece, nil, Count) )
    else
      Check( SDOra.osetpi(PCdaDef(Handle), Piece, Buffer, Count) );
    rcd := SDOra.oexec(PCdaDef(Handle));
    if rcd = 0 then
      Break
    ...
    if IsBlobType(Params[i].DataType) then begin
      if DetectedOCI73 and UsePiecewiseCalls then
        ...
        Check( obndra(PCdaDef(Handle), szSqlVar, -1,
        		{$IFDEF SD_CLR}BufList.StringToPtr(Params[i].AsString){$ELSE}TSDCharPtr(TVarData(Params[i].Value).VString){$ENDIF},
...
{$ENDIF}
    ...
    Check( SDOra.oparse(PCdaDef(Handle), szCmd, -1, 0, ORACLE7_PARSE) );
...
{$ELSE}
    szCmd := TSDCharPtr(FStmt);
{$ENDIF}
    Check( SDOra.oparse(PCdaDef(Handle), szCmd, -1, DEFERRED_PARSE, ORACLE7_PARSE) );
  end else
    DatabaseError(SNoCapability);
...
begin
  if CommandType in [ctQuery, ctStoredProc] then begin
    ...
    Check( SDOra.oexec(PCdaDef(Handle)) );
end;
    ...
    if Assigned(Params) then
      for i:=0 to Params.Count-1 do
        if IsSupportedBlobTypes(Params[i].DataType) then begin
          ...
          WriteBlobByName(Params[i].Name, {$IFDEF SD_CLR}BufList.StringToPtr(sBlob){$ELSE}TSDCharPtr(sBlob){$ENDIF}, Length(sBlob));
        end;
  ...
  try
    ...
    for i := 0 to Params.Count - 1 do begin
      ...
      if Params[i].ParamType in [ptResult, ptInputOutput, ptOutput] then begin
        if Params[i].DataType = ftCursor then begin
          if FParamHandle = nil then
            FParamHandle := {$IFDEF SD_CLR} IncPtr(FldBuf, SizeOf(TSDFieldInfo)) {$ELSE} PCdaDef( FldBuf + SizeOf(TSDFieldInfo) ) {$ENDIF};
        end else begin
...
var
  rcd: sword;
  szCmd: TSDCharPtr;
  acArgname:	{$IFDEF SD_CLR} TSDCharPtr {$ELSE} array[0..MaxSPParamCount-1, 0..MaxSPParamName-1] of Char {$ENDIF};
  ab2Ovrld, ab2Pos, ab2Level, ab2Arnlen,
  ab2DType, ab2Prec, ab2Scale:	{$IFDEF SD_CLR} sb2p {$ELSE}array[0..MaxSPParamCount-1] of sb2 {$ENDIF};
  ab1Defsup, ab1Mode, ab1Radix:	{$IFDEF SD_CLR} ub1p {$ELSE}array[0..MaxSPParamCount-1] of ub1 {$ENDIF};
  ab4Dtsize, ab4Spare:	        {$IFDEF SD_CLR} ub4p {$ELSE}array[0..MaxSPParamCount-1] of ub4 {$ENDIF};
  arrsize: ub4;
  i: Integer;
  ft: TFieldType;
  ...
  begin
    Result := {$IFDEF SD_CLR} '' {$ELSE} Copy(acArgname[Index], 0, ab2Arnlen[Index]) {$ENDIF};
  end;
  function GetArgPos(Index: Integer): sb2;
  begin
    Result := {$IFDEF SD_CLR} HelperMemReadInt16(ab2Pos, Index*SizeOf(Result)) {$ELSE} ab2Pos[Index] {$ENDIF};
  end;
  function GetArgDType(Index: Integer): sb2;
  begin
    Result := {$IFDEF SD_CLR} HelperMemReadInt16(ab2DType, Index*SizeOf(Result)) {$ELSE} ab2DType[Index] {$ENDIF};
  end;
  function GetArgPrec(Index: Integer): sb2;
  begin
    Result := {$IFDEF SD_CLR} HelperMemReadInt16(ab2Prec, Index*SizeOf(Result)) {$ELSE} ab2Prec[Index] {$ENDIF};
  end;
  function GetArgScale(Index: Integer): sb2;
  begin
    Result := {$IFDEF SD_CLR} HelperMemReadInt16(ab2Scale, Index*SizeOf(Result)) {$ELSE} ab2Scale[Index] {$ENDIF};
  end;
  function GetArgMode(Index: Integer): ub1;
  begin
    Result := {$IFDEF SD_CLR} HelperMemReadByte(ab1Mode, Index*SizeOf(Result)) {$ELSE} ab1Mode[Index] {$ENDIF};
  end;
...
{$IFDEF SD_CLR}
  ...
  try
    ...
    rcd := SDOra.odessp( PCdaDef( SqlDatabase.LdaPtr ), szCmd, -1, nil, 0, nil, 0,
    ...
    if IsBlobType(Params[i].DataType) then begin
      ...
      if dwLoadedOCI >= MakeVerValue(7, 3) then
        ...
        if VarType(Params[i].Value) <> varString then
          VarAsType(Params[i].Value, varString);
        Check( obndra(PCdaDef(Handle), szSqlVar, -1,
        		{$IFDEF SD_CLR}BufList.StringToPtr(Params[i].AsString){$ELSE}TSDCharPtr(TVarData(Params[i].Value).VString){$ENDIF}, Params[i].GetDataSize,
      			NativeDataType(Params[i].DataType), -1,
                      	IncPtr(CurPtr, GetFieldInfoFetchStatusOff), nil, nil, 0, nil, nil, 0, 0) );
      end;
    end else begin
...
begin
  ...
  Check( SDOra.oclose( PCdaDef(FParamHandle) ) );
  FParamHandle := nil;
end;
...
begin
  ...
  if not FBoundParams then
	...
	// get column number
  Check( OCIAttrGet( Pdvoid(Handle), OCI_HTYPE_STMT, {$IFNDEF SD_CLR}@{$ENDIF}NumCols, nil, OCI_ATTR_PARAM_COUNT, ErrPtr) );
  ...
  for i:=1 to NumCols do begin
    ...
    Check( OCIAttrGet( Pdvoid(phCol), OCI_DTYPE_PARAM,
    			 {$IFDEF SD_CLR}szColName, ColNameLen{$ELSE}dvoid(@szColName), @ColNameLen{$ENDIF}, OCI_ATTR_NAME, ErrPtr) );
    Check( OCIAttrGet( Pdvoid(phCol), OCI_DTYPE_PARAM,
    			 {$IFNDEF SD_CLR}@{$ENDIF}dtype, nil, OCI_ATTR_DATA_TYPE, ErrPtr) );
    Check( OCIAttrGet( Pdvoid(phCol), OCI_DTYPE_PARAM,
    			 {$IFNDEF SD_CLR}@{$ENDIF}dsize, nil, OCI_ATTR_DATA_SIZE, ErrPtr) );
    Check( OCIAttrGet( Pdvoid(phCol), OCI_DTYPE_PARAM,
    			 {$IFNDEF SD_CLR}@{$ENDIF}prec, nil, OCI_ATTR_PRECISION, ErrPtr) );
    Check( OCIAttrGet( Pdvoid(phCol), OCI_DTYPE_PARAM,
    			 {$IFNDEF SD_CLR}@{$ENDIF}dscale, nil, OCI_ATTR_SCALE, ErrPtr) );
    Check( OCIAttrGet( Pdvoid(phCol), OCI_DTYPE_PARAM,
    			 {$IFNDEF SD_CLR}@{$ENDIF}NullOk, nil, OCI_ATTR_IS_NULL, ErrPtr) );
    sColName := {$IFDEF SD_CLR} Marshal.PtrToStringAnsi( szColName, MinIntValue(ColNameLen, MaxColNameLen) ) {$ELSE} Copy( szColName, 1, MinIntValue(ColNameLen, MaxColNameLen) ) {$ENDIF};
    ...
    if dtype = SQLT_RDD then begin
      Check( OCIAttrGet( Pdvoid(phCol), OCI_DTYPE_PARAM,
    			 {$IFNDEF SD_CLR}@{$ENDIF}dispsize, nil, OCI_ATTR_DISP_SIZE, ErrPtr) );
      dsize := dispsize;
    end;
...
begin
  ...
  if not Assigned(FHandle) then
    Exit;
  Check( OCIAttrGet( Pdvoid(FHandle), OCI_HTYPE_STMT, {$IFNDEF SD_CLR}@{$ENDIF}StmtType, nil, OCI_ATTR_STMT_TYPE, ErrPtr) );
      ...
      if not IsBlobType(DataType) then begin
        ...
        if IsOra8BlobType(DataType) then begin
          if TempLobSupported then begin
            ...
            WriteLobData(phLob, {$IFDEF SD_CLR}BufList.StringToPtr(Params[i].AsString){$ELSE}TSDCharPtr(TVarData(Value).VString){$ENDIF}, RealParamDataSize(Params[i]));
...
//              FillChar( aBindLobs^, Params.Count * SizeOf(TBindLobInfo), 0 );
		...
		// count of Lob parameters
          ...
          Check( OCIBindByName( Handle, phBind, ErrPtr, szSqlVar, Length(sqlvar),
        	{$IFDEF SD_CLR}BufList.StringToPtr(Params[i].AsString){$ELSE}dvoid(TSDCharPtr(TVarData(Value).VString)){$ENDIF}, RealParamDataSize(Params[i]),
...
{$ENDIF}
    ...
    if Assigned(Params) and not TempLobSupported and {$IFDEF SD_VCL5} True {$ELSE} SqlDatabase.IsOra8BlobMode {$ENDIF} then begin
      i := 0;
...
begin
  ...
  if iters = 0 then begin
    ...
    Check( OCIAttrSet( Pdvoid(Handle), OCI_HTYPE_STMT, {$IFNDEF SD_CLR}@{$ENDIF}PrefetchRows, SizeOf(PrefetchRows), OCI_ATTR_PREFETCH_ROWS, ErrPtr) );
  end;
...
begin
  ...
  Check( OCIAttrGet( Pdvoid(FHandle), OCI_HTYPE_STMT, {$IFNDEF SD_CLR}@{$ENDIF}rows, nil, OCI_ATTR_ROW_COUNT, ErrPtr) );
  Result := rows;
end;
...
{$ENDIF}
	...
	// Get the parameter for the package
      Check( OCIAttrGet( Pdvoid(phDesc), OCI_HTYPE_DESCRIBE, {$IFNDEF SD_CLR}@{$ENDIF}phParm, nil, OCI_ATTR_PARAM, ErrPtr) );
      	// get parameter for list of subprograms
      Check( OCIAttrGet( Pdvoid(phParm), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}phSubLst, nil, OCI_ATTR_LIST_SUBPROGRAMS, ErrPtr) );
	//  How many subroutines are in this package ?
      Check( OCIAttrGet( Pdvoid(phSubLst), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}NumSubs, nil, OCI_ATTR_NUM_PARAMS, ErrPtr) );
      i := 0;
	...
	// Get the routine type and name...
        Check( OCIAttrGet( Pdvoid(phParm), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}SubType, nil, OCI_ATTR_PTYPE, ErrPtr) );
        IsFunc := SubType = OCI_PTYPE_FUNC;
        szName := nil;
        Check( OCIAttrGet( Pdvoid(phParm), OCI_DTYPE_PARAM, {$IFDEF SD_CLR}szName, NameLen{$ELSE}@szName, @NameLen{$ENDIF}, OCI_ATTR_NAME, ErrPtr) );
       	// the routine's found
...
{$ENDIF}
	...
	// get the parameter handle
      ...
      Check( OCIAttrGet( Pdvoid(phDesc), OCI_HTYPE_DESCRIBE, {$IFDEF SD_CLR}phParm, NameLen{$ELSE}@phParm, nil{$ENDIF}, OCI_ATTR_PARAM, ErrPtr) );
    end;

	// Get the number of arguments and the arg list
    NameLen := 0;
    Check( OCIAttrGet( Pdvoid(phParm), OCI_DTYPE_PARAM, {$IFDEF SD_CLR}phArgLst, NameLen{$ELSE}@phArgLst, nil{$ENDIF},
    			OCI_ATTR_LIST_ARGUMENTS, ErrPtr) );
    Check( OCIAttrGet( Pdvoid(phArgLst), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}NumArgs, nil,
    ...
    while i <= NumArgs do begin
      ...
      szName := nil;
      Check( OCIAttrGet( Pdvoid(phArg), OCI_DTYPE_PARAM, {$IFDEF SD_CLR}szName, NameLen{$ELSE}@szName, @NameLen{$ENDIF},
      				OCI_ATTR_NAME, ErrPtr) );
      Check( OCIAttrGet( Pdvoid(phArg), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}dtype, nil,
      				OCI_ATTR_DATA_TYPE, ErrPtr) );
      Check( OCIAttrGet( Pdvoid(phArg), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}iomode, nil,
      				OCI_ATTR_IOMODE, ErrPtr) );
      Check( OCIAttrGet( Pdvoid(phArg), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}prec, nil,
      				OCI_ATTR_PRECISION, ErrPtr) );
      Check( OCIAttrGet( Pdvoid(phArg), OCI_DTYPE_PARAM, {$IFNDEF SD_CLR}@{$ENDIF}scale, nil,
    ...
    if IsBlobType(DataType) then begin
      if IsOra8BlobType(DataType) then begin
        ...
        if ParamType in [ptInput, ptInputOutput] then begin
          ...
          LobEmpty := 0;
          Check( OCIAttrSet(Pdvoid(phLob), OCI_DTYPE_LOB, {$IFNDEF SD_CLR}@{$ENDIF}LobEmpty, SizeOf(LobEmpty), OCI_ATTR_LOBEMPTY, ErrPtr) );
        end;
        HelperMemWritePtr( DataPtr, 0, phLob );         // POCILobLocator(DataPtr^) := phLob;
        phLob	:= nil;
        Check( OCIBindByName( Handle, phBind, ErrPtr, szSqlVar, Length(sqlvar),
