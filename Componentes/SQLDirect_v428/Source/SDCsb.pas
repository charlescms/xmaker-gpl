...
begin
  Result := SDCsb.IsVersion7 and IsTransLogging;
end;
...
function TICsbDatabase.ParamValue(Value: TSDDatabaseParam): Integer;
begin
  case Value of
...
var
  ...
  OldDateSep, OldTimeSep:{$IFDEF SD_CLR} string {$ELSE} Char {$ENDIF};
...
{$ENDIF}
    ...
    if IsBlobType(DataType) then begin
      sBlob := AsBlob;
      WriteBlobByName(Name, {$IFDEF SD_CLR}BufList.StringToPtr(sBlob){$ELSE}PChar(sBlob){$ENDIF}, Length(sBlob));
    end else
