...
begin
  ...
  FMaxFieldNameLen	:= StrToIntDef( Trim( Params.Values[szMAXFIELDNAMELEN] ), SDCommon.MAXFIELDNAMELEN );
  FMaxStringSize	:= StrToIntDef( Trim( Params.Values[szMAXSTRINGSIZE] ), SDCommon.DEFMAXFIELDSTRINGSIZE );
  FBlobPieceSize	:= StrToIntDef( Trim( Params.Values[szBLOBPIECESIZE] ), SDCommon.DEF_BLOB_PIECE_SIZE );
...
{ It's used in TSDStoredProc.DescriptionsAvailable method and when TSDStoredProc.Params property is clicked in Object Inspector }
...
function TISqlDatabase.ParamValue(Value: TSDDatabaseParam): Integer;
begin
...
begin
  with Param do
    ...
    if RequiredCnvtFieldType(DataType) {$IFDEF SD_VCL5} or (DataType = ftGuid){$ENDIF} then
